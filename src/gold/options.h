...
class General_options
{
 private:
  ...
  // xie. 添加update_shuffled时的控制选项
  DEFINE_bool(print_layout, options::TWO_DASHES, '\0', false,
            N_("在update_shuffled函数中打印layout"), NULL);
  ...
  // r
    /// xie: 需要针对该情况进行处理，保留并修改重定位，同时链接器的元数据聚合需要能够接受objtemp
