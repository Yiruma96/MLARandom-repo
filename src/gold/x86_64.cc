...
class Target_x86_64 : public Sized_target<size, false>
{
 ...
 private:
  ...
  class Relocate
  {
   ...
   private:
    ...
    relocate_tls(const Relocate_info<size, false>*, Target_x86_64*,
		 ...
		 unsigned char*, typename elfcpp::Elf_types<size>::Elf_Addr,
		 section_size_type,
         ShuffleInfo::Metadata_FixupInfo* fixup_tuple);
  ...
  optimize_tls_reloc(bool is_final, int r_type, Layout* layout); /// xie:
...
Target_x86_64<size>::optimize_tls_reloc(bool is_final, int r_type, Layout* layout)
{
...
//    }
    ...
    /// xie: 静态链接下和TLS优化不兼容，所以遇到静态链接时我们取消元数据合并同时允许TLS优化。日后如果实现了TLS兼容的话就无法care静态链接了。
    /// 实验发现静态链接时禁用TLS优化的话，会导致一些dymsym没有被设置idx，从而在write_section写reloc前对reloc排序读取其symidx的时候报错。
    if (!parameters->doing_static_link()){
        return tls::TLSOPT_NONE;
    }
			    ...
			    Symbol* gsym)
{
  ...
  switch (r_type)
	...
	const tls::Tls_optimization optimized_type
	    = Target_x86_64<size>::optimize_tls_reloc(is_final, r_type, layout); /// xie:
	switch (r_type)
	  {
	  case elfcpp::R_X86_64_TLSGD:       // General-dynamic
	    if (optimized_type == tls::TLSOPT_NONE)
 ...
 public:
		 ...
		 typename elfcpp::Elf_types<64>::Elf_Addr address)
  {
    ...
    if (addend >= 0) {
        ...
        if (need_update || need_add) {
            std::pair<Valtype, elfcpp::Elf_types<64>::Elf_Swxword> temp = psymval->template getValueAddend(object, addend);
...
/*
(x64版本)关于什么时候需要重新need_update的结论如下：

 need_update用于精确收集基址为got和tls的target和base，以便于应对组合重定位的情况。x64下组合重定位不存在因此无需做精确收集。

 但我们还是考虑一下：更新后会使原始等式变为汇编器优化后的第二等式，因此可能存在base从SYM变为PC，target从Normal变为节的情况，此时会导致随机化问题。
 之前将重定位范围限制在了GOT和TLS类型的重定位中，但还需要进一步过滤：
 - 比如R_X86_64_GOTOFF64，其target为normal，如果重新更新的话，target被错误的确定为section地址，随机化的时候就有问题了。过滤掉
 - 比如R_X86_64_GOTPC64，其base在编译阶段收集为符号，如果重新更新的话，base被错误的确定为pc地址，随机化的时候就有问题了。过滤掉
 最后剩下的就是可以精确收集且第二等式和第一等式一致的。


 NOTE: 虽然x64不需要精确收集，但还是有其他需求导致需要need update的：
    和plt一样，当指针的target为TLS Entry或GOT Entry，其target同样需要改变。此外gas会将push f1@GOTPCREL(%rip)的基址错误的认为是GOT_OFFSET_TABLE,
    因此TLS和GOT类型的指针最好还是要need_update下，但这最终不可避免的遇到第二等式和第一等式的不一致问题。。。
 * */
    ...
    section_size_type view_size)
{
  ...
  /// xie:
  uint64_t fixup_offset = address - os->address();
...
//    printf("%s %lx\n", object->name().c_str(), fixup_offset);

  // Pick the value to use for symbols defined in the PLT.
    /// 这里会遍历每个重定位项，在开启随机化开关时，给其对应的fixup打上重定位的标记
    /// FIXME: 这里的搜索设计有问题，首先没有考虑.data.rel.ro.local，其次也没有考虑.fini_array  建议就采用一个数组，或是像gas那样采用两层数组
    ShuffleInfo::Metadata_FixupInfo* fixup_tuple = nullptr;
    if (need_output_rand && relinfo->object->has_rand){
    ...
    /// 三处target地址的修正. 当输出含有元数据时(need_rand_merge)，修改fixup的值
    bool update_psymval = false;
    Symbol_value<size> symval;
    /// target地址由于plt符号调整发生了改变。
    /// 注意在521.wrf_r中，存在有wrf_dm_on_monitor_@PLT，并且重定位类型为R_X86_64_PLT32，但并没有将符号重新指向.plt节的情况。所以目前这种方案是正确的，只收集的确发生改变的。
    if (gsym != NULL && gsym->use_plt_offset(Scan::get_reference_flags(r_type))) {
        if (need_output_rand && fixup_tuple != nullptr)
            my_layout->plt_target += 1;
  ...
  switch (r_type)
    {
    /// xie: 虽然和R_X86_64_GOTPCREL那三个系出同源，但gold不转换该重定位，因此可以首先从sym计算出GOT ENTRY的地址
    case elfcpp::R_X86_64_GOT32:
    case elfcpp::R_X86_64_GOT64:
    case elfcpp::R_X86_64_GOTPLT64:
    case elfcpp::R_X86_64_GOTPCREL64:
  ...
  switch (r_type)
    ...
    case elfcpp::R_X86_64_PLT32:    // 基址为pc，target为PLT entry
    case elfcpp::R_X86_64_PLT32_BND:
      gold_assert(gsym == NULL
		  || gsym->has_plt_offset()
		  || gsym->final_value_is_known()
    ...
    case elfcpp::R_X86_64_PLTOFF64: // 基址为got.plt也就是GOT_OFFSET_TABLE, target为psymval(在前面该符号已经做转换了)
      {
	gold_assert(gsym);
	gold_assert(gsym->has_plt_offset()
		    || gsym->final_value_is_known());
	...
	Reloc_funcs::pcrela64(view, object, psymval, addend, got_address);
      }
      break;

    case elfcpp::R_X86_64_GOT32: // 同，见下面的R_X86_64_GOT64
        if (need_output_rand && fixup_tuple != nullptr){ // xie:
            my_layout->got_target += 1;
        }
      gold_assert(have_got_offset);
      Reloc_funcs::rela32(view, got_offset, addend);
      break;

    case elfcpp::R_X86_64_GOTPC32: // 同，R_X86_64_GOTPC64
      {
	gold_assert(gsym);
	typename elfcpp::Elf_types<size>::Elf_Addr value;
    ...
    case elfcpp::R_X86_64_GOT64: // 基址为GOT_OFFSET_TABLE target为GOT Entry。只是这里直接计算出了got_offset因此用的rela64
    case elfcpp::R_X86_64_GOTPLT64: // 这两个重定位的base和target均不在随机化范围内因此不做重新收集
      // R_X86_64_GOTPLT64 is obsolete and treated the same as GOT64.
      if (need_output_rand && fixup_tuple != nullptr){ // xie:
          my_layout->got_target += 1;
      }
      gold_assert(have_got_offset);
    ...
    // xie: 基址为address中记录的pc,target为GOT_OFFSET_TABEL. 注意该指针的基址可能是SYM(回忆pie-large下计算GOT_OFFSET_TABLE的过程)
    // 因此该重定位不要做重新收集不然会破坏基址
    case elfcpp::R_X86_64_GOTPC64:
      {
	gold_assert(gsym);
	typename elfcpp::Elf_types<size>::Elf_Addr value;
    ...
    // xie: got类重定位的重新收集
    if (need_output_rand && fixup_tuple != nullptr){
        fixup_tuple->set_target_bbl_sym(value);
    }
      }
      break;

    case elfcpp::R_X86_64_GOTOFF64: // 其基址是GOT_OFFSET_TABEL，target是normal
      {
    ...
    // xie: got类重定位的重新收集
    if (need_output_rand && fixup_tuple != nullptr){
        fixup_tuple->set_base_bbl_sym(reladdr);
    }
      }
      break;

    case elfcpp::R_X86_64_GOTPCREL:   // 这三个基址为address, target在未优化时为GOT Entry，优化后为Normal
    case elfcpp::R_X86_64_GOTPCRELX:
    case elfcpp::R_X86_64_REX_GOTPCRELX:
      {
      bool converted_p = false;

      /// xie: 对转换导致的情况不太清楚，因此先暂时禁用这些转化
      if (rela.get_r_addend() == -4)
	{
	  ...
	  // if possible.
	  /// xie: 该转换的说明：
	  /// 该转换希望将一个对got地址的引用转为直接对确定符号地址的引用，当然这取决于该符号的位置在静态链接时是否可确定以及是否位于动态库中
	  ///
	  /// exp1: 地址在静态连接时不确定，保留对got项的指向+为got项增加一个动态重定位  /lib/x86_64-linux-gnu/crti.o
	  /// 000000000000000b  000000080000002a R_X86_64_REX_GOTPCRELX 0000000000000000 __gmon_start__ - 4
	  /// __gmon_start__符号在静态链接时不可知，因此直接指向它，只能指向存储该符号地址的got地址。修复后如下：
	  /// mov    0x1b19(%rip),%rax        # 401fe0 <__gmon_start__>   可以看到指向了<__gmon_start__>的got项
	  /// 相应的为该got增加一个动态重定位
	  /// 0000000000401fe0  0000000700000006 R_X86_64_GLOB_DAT      0000000000000000 __gmon_start__ + 0
	  ///
	  /// exp2: 地址在静态连接时可以确定，因此直接改为引用真实地址  /lib/x86_64-linux-gnu/crt1.o
	  /// 0000000000000016  000000090000002a R_X86_64_REX_GOTPCRELX 0000000000000000 __libc_csu_fini - 4
	  /// __libc_csu_fini符号属于其他.o文件的，因此该符号在链接时可确定，直接指向它
	  /// lea    0x2b6(%rip),%r8        # 4007f0 <__libc_csu_fini>   可以看到直接指向了<__libc_csu_fini>的真实地址
	  ///
	  /// 补充：在gcc中，对于一些gcc觉得即便静态也可能确定不了的，就会用sym@got，表示将符号地址放到got中，然后对该got的引用
	  /// 当然gcc认为大部分符号都是可以在静态链接时确定的，所以用jmp sym就表示直接对该符号所在位置的引用
	  if (!parameters->incremental()
	      && ((gsym == NULL
		   && rela.get_r_offset() >= 2
		   && view[-2] == 0x8b
	    ...
	    {
	      view[-2] = 0x8d;
	      new_reloc = elfcpp::R_X86_64_PC32; /// xie: 指向got的变为指向任意地址的
          ...
          if (need_output_rand && fixup_tuple != nullptr){
              ...
              _type &= 0x0000ffff;
              _type |= new_reloc << 16;
              fixup_tuple->set_type(_type);
          }

	    }
	  /// 这个转换也是把对got的引用转化为直接对真实地址的引用
	  // Convert
	  // callq *foo@GOTPCRELX(%rip) to
	  // addr32 callq foo
	  // and jmpq *foo@GOTPCRELX(%rip) to
	  ...
	  // nop
	      ...
	      else
		{
		  ...
		  // by 1 byte.
		  new_address -= 1; /// xie: 指令调整后，最后一个字节变为nop，因此相当于指令提前一个字节结束，基址要-1
          ...
          if (need_output_rand && fixup_tuple != nullptr){
              ...
              _type |= new_reloc << 16;
              fixup_tuple->set_type(_type);
              fixup_tuple->set_offset(new_address);
          }
		}
	    }
	}

        /// xie: 如果不做转化的话，基址为pc，目标地址为GOT+got_offset
      if (!converted_p)
	{
        if (need_output_rand && fixup_tuple != nullptr)
            my_layout->got_target += 1; // xie:
	  if (gsym != NULL)
	    {
	      gold_assert(gsym->has_got_offset(GOT_TYPE_STANDARD));
	      /// xie: got地址计算示例：
	      /// gsym->got_offset(GOT_TYPE_STANDARD) got区域的前半部分长度为8
	      ///target->got_size() =16               整个got区域的长度为16
	      /// got_offset = -8                     got区域的后半部分长度为-8
	      /// 用.got.plt + got_offset 即可得到got项的地址
    ...
    case elfcpp::R_X86_64_GOTPCREL64: // 基址为pc, target为GOT Entry
      {
    ...
    case elfcpp::R_X86_64_GOTTPOFF:         // Initial-exec
    case elfcpp::R_X86_64_TPOFF32:          // Local-exec
        need_update = need_output_rand; /// xie:
      this->relocate_tls(relinfo, target, relnum, rela, r_type, gsym, psymval,
			 view, address, view_size, fixup_tuple);
      break;
    ...
    if (need_output_rand && need_update && fixup_tuple != nullptr){
        if (has_record){
            fixup_tuple->set_target_bbl_sym(target_address);
            ...
            unsigned int _type = fixup_tuple->type();
            if ((_type & 0x3) > 1) // 如果基址是pc或是none，这两者不会改变因此也无需进一步的精确记录
            ...
            printf("target位于plt或got中，但没有在重定位处理函数中找到它们的值\n");
            exit(0x233);
        }
    }

    /// xie: 加入第一波的fixup
    if (need_output_rand && need_add) {
        if (has_record){
            /// 收集自重定位函数，因为有一些使用psymval(obj, add)拿到值，所以target里是包含add的，所以add参数置为固定的0
            my_layout->add_other_fixup(new_address, os, target_address, base_address, target_add, new_reloc, 1);
        }else{
    ...
    ShuffleInfo::Metadata_FixupInfo* fixup_tuple)
{
  ...
  tls::Tls_optimization optimized_type
      = Target_x86_64<size>::optimize_tls_reloc(is_final, r_type, NULL); /// xie:

    // xie: 统计tls优化数量
    if (need_output_rand && optimized_type != tls::TLSOPT_NONE && fixup_tuple != nullptr){
        my_layout->tls_opt += 1;
    }
      ...
      else
	{
	  ...
	  if (optimized_type == tls::TLSOPT_TO_IE)
	    {
          if (need_output_rand && fixup_tuple != nullptr) {
              my_layout->tls_got_target += 1; // xie:
          }
	      value = target->got_plt_section()->address() + got_offset;
