...
/// xie: 在gold处理重定位时收集base，target，add三个值
static bool need_output_rand = false;
static bool need_update = false;
static bool need_add = false;
static bool has_record = false;
static uint64_t target_address = 0;
static int64_t target_add = 0;
static uint64_t base_address = 0;
static unsigned int new_reloc = 0;
static uint64_t new_address = 0;
 ...
 private:
  ...
  rel(unsigned char* view, Address value, Overflow_check check)
  {
    ...
    Valtype addend = elfcpp::Swap<valsize, big_endian>::readval(wv);
      ...
      if (need_update || need_add) {
          target_add = (typename elfcpp::Valtype_base<valsize>::Signed_valtype)addend;
      ...
      Overflow_check check)
  {
    ...
    Valtype addend = elfcpp::Swap<valsize, big_endian>::readval(wv);
      ...
      /// xie:
      if (need_update || need_add) {
          std::pair<Valtype, Valtype> temp = psymval->template getValueAddend(object, addend);
          target_address = temp.first;
          target_add = (typename elfcpp::Valtype_base<valsize>::Signed_valtype)temp.second;
          has_record = true;
      }
                ...
                Overflow_check check)
  {
    ...
    Valtype addend = elfcpp::Swap_unaligned<valsize, big_endian>::readval(view);
      ...
      /// xie:
      if (need_update || need_add) {
          std::pair<Valtype, Addendtype> temp = psymval->template getValueAddend(object, addend);
          target_address = temp.first;
          target_add = (typename elfcpp::Valtype_base<valsize>::Signed_valtype)temp.second;
          has_record = true;
      }
