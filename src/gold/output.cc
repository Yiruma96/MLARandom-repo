...
Output_data_got<got_size, big_endian>::Got_entry::write(
    ...
    unsigned char* pov, Layout* my_layout, Output_section* os, uint64_t got_offset) const
{
    bool need_record = false; /// xie:
  Valtype val = 0;
  ...
  switch (this->local_sym_index_)
	...
	if (this->use_plt_or_tls_offset_ && gsym->has_plt_offset()) {
	    need_record = true; /// xie: global符号的plt
	...
	else
	  {
	    switch (parameters->size_and_endianness())
	      ...
	      case Parameters::TARGET_32_BIG:
		{
		    need_record = true; /// xie: global符号的值
	...
	if (this->use_plt_or_tls_offset_ && !is_tls) {
        need_record = true; /// xie: local符号的plt    plt可能也指向随机化区域，但目前存疑的是plt指的是offset还是函数的绝对地址，如果是offset则可能被误认为指向随机化区域导致错误
	...
	else
	  {
	    ...
	    need_record = true; /// xie: local符号的值
	    if (this->use_plt_or_tls_offset_ && is_tls) {
	        need_record = false;
  ...
  /// xie: .got.plt节的不收集，其my_layout为NULL
  if (my_layout && need_record) {

      /// 确定r_type
      unsigned int r_type = 0;
      if (got_size == 64 && my_layout->shuffle_info_.arch == 1){
          r_type = 1;   // elfcpp::R_X86_64_64;
      } else if (got_size == 64 && my_layout->shuffle_info_.arch == 2){
          r_type = 257; // elfcpp::R_AARCH64_ABS64
      } else if (got_size == 32 && my_layout->shuffle_info_.arch == 1){
          r_type = 10;  // elfcpp::R_X86_64_32;
      } else if (got_size == 32 && my_layout->shuffle_info_.arch == 2){
          r_type = 258; // elfcpp::R_AARCH64_ABS32;
      } else{
          printf("无法确定got指针的r_type  got_size=%d arch=%d\n", got_size, my_layout->shuffle_info_.arch);
          exit(0x233);
      }

      /// 在otherfixups中创建新的fixup
      my_layout->add_other_fixup(got_offset, os, val, 0, 0, r_type, 2);
  }
...
Output_data_got<got_size, big_endian>::do_write(Output_file* of)
{
  ...
  /// xie: 从output_section中拿到layout，供加入got节的指针
  Output_section* os = this->output_section();
  Layout* my_layout = NULL;
  if (os && !strcmp(".got", os->name())){
      my_layout = os->get_layout();
      if (my_layout->need_output_rand == false)
          my_layout = NULL; // 未随机化时不收集
  }
  ...
  for (unsigned int i = 0; i < this->entries_.size(); ++i)
    {
      this->entries_[i].write(i, pov, my_layout, os, got_offset);
      got_offset += add;
      pov += add;
    }
    ...
    reloc_section_(NULL)
{
  ...
  // sections.
  this->my_layout = my_layout; /// xie: output_section是Layout的时候创建的，因此在os中记录下Layout以方便拿存储的元数据
				  ...
				  bool have_sections_script)
{
  ...
  if (aligned_offset_in_section > offset_in_section
      ...
      && parameters->target().has_code_fill())
    {
      /// 填充数据由自定义的Fill给出，当parameters->has_code_fill时
	...
	{
      /// 否则填充数据由默认的code_fill函数给出. 跟入arch.cc-do_code_fill()函数，可以看到以下填充规则
      /// - x86_64.cc. 大于16时用jmp+\x90,小于16时用\x66那种预选择数组
      /// - arm.cc 没有找到do_code_fill函数。。。可能是arm程序不需要对齐？？？
	  std::string fill_data(parameters->target().code_fill(fill_len));
	  Output_data_const* odc = new Output_data_const(fill_data, 1);
      const char* fill_data_char = fill_data.c_str();

      /// xie: 打印填充的数据 (可以学习下code_fill函数是怎么填充input_section间的数据的，我是因为注意到他用jmp end;\x90;\x90...填充才来分析的)
//      if (!strcmp(".text", this->name())){
//          gold_info("ADD a non-merge input section to .text: %s %s", object->sized_relobj()->name().c_str(), secname);
//          std::string content_str = odc->data_;
//          const char* content_char = odc->data_.c_str();
//          int content_length = content_str.length();
//          char content_hex[content_length*3+1];
//          memset(content_hex, 0, content_length*3+1);
//          for (int str_idx=0, hex_idx=0; str_idx<content_length; str_idx++, hex_idx+=3) {
//              snprintf(content_hex + hex_idx, 4, "%.2X ", (uint8_t)content_char[str_idx]);
//          }
//          content_hex[content_length*3] = '\x00';
//          gold_info("Fill Data: %s", content_hex);
//      }

      Input_section isecn(odc);
      /// xie: 在向OutputSection中加入InputSection的时候，记录所创建的input_section的名字
      isecn.record_input_section_name(str_concat("fill-", secname));
  ...
  if (this->always_keeps_input_sections_
      ...
      || layout->is_section_ordering_specified())
    {
      ...
      /// xie: 打印填充的数据
//      if (!strcmp(".text", this->name())){
//          gold_info("ADD a ordinary input section to .text: %s %s", object->sized_relobj()->name().c_str(), secname);
//      }
      /// xie: 在向OutputSection中加入InputSection的时候，记录所创建的input_section的名字
...
//                    gold_info("    [SOFT END] %s %s off=0x%lx size=0x%lx",
//                              p->get_input_section_name(), p->relobj()->name().c_str(), off, p->data_size());
//                }else{
//                    gold_info("    [SOFT END] [NOT_INPUT_SECTION] %s off=0x%lx size=0x%lx",
//                              p->get_input_section_name(), off, p->data_size());
//                }
...
Output_section::sort_attached_input_sections()
{
  ...
  if (this->must_sort_attached_input_sections())
	...
	std::sort(sort_list.begin(), sort_list.end(),
          ...
          std::sort(sort_list.begin(), sort_list.end(), Input_section_sort_section_prefix_special_ordering_compare());
      }
      else
	std::sort(sort_list.begin(), sort_list.end(),
  ...
  for (int i = 0; i < static_cast<int>(ORDER_MAX); ++i)
      ...
      if (i == static_cast<int>(ORDER_RELRO_LAST))
	{
	  *poff += last_relro_pad;
      ...
      /// xie. 遍历OutputSection中的每个OutputData,应该一个OutputData对应一个OutputSection吧
      ...
      if (!(*p)->is_address_valid())
	{
	  ...
	  if (!parameters->incremental_update())
		...
		foff = off;
          ...
          /// xie: 为OutputData中的每个InputSection确定Offset和Size
          // gold_info("set_address_and_file_offset [%s] addr=0x%lx off=0x%lx", (*p)->do_output_section()->name(), addr, off);
