...
/// xie: 字符串拼接
inline char* str_concat(const char* str1, const char* str2){
    int str1_size = strlen(str1);
    int str2_size = strlen(str2)+1; // 字符串的末尾借用str2的末尾
    char* new_str = (char*)malloc(str1_size + str2_size + 1);
    memcpy(new_str, str1, str1_size);
    new_str[str1_size] = ' ';
    memcpy(new_str+str1_size+1, str2, str2_size);
    return new_str;
}
