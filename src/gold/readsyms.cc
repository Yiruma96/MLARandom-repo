...
/// xie: 从不同版本的ehdr中获得e_machine
template<int size, bool big_endian>
static inline unsigned int get_e_machine(elfcpp::Ehdr<size, big_endian> elf_header){
    return elf_header.get_e_machine();
}
  ...
  if (is_elf)
    {
      ...
      /// xie: 这里应该是对所有待链接的file第一次生成架构对应的sized_object，所以在这里确定本次链接的架构类型
      /// 跟进后可以看到object对象先根据elf头的e_machine字段确定target。 然后调用target->make_elf_object()来生成的object对象
      /// 我们不改动这里，因为ehdr现在还是字符串，需要基于头部信息才能转化为ehdr。我们在更深层等ehdr确定后再修改
      ...
      int size = 0;
      std::string error;
      unsigned int e_machine = -1;
      if (!elfcpp::Elf_recognizer::is_valid_header(ehdr, read_size, &size, &big_endian, &error)){
          gold_error(_("%s: %s"), input_file->filename().c_str(), error.c_str());
          exit(0x233);
      }
      if (size == 32) {
          if (big_endian) {
              elfcpp::Ehdr<32, true> elf_header = ehdr;
              e_machine = get_e_machine<32, true>(elf_header);
          }
          else {
              elfcpp::Ehdr<32, false> elf_header = ehdr;
              e_machine = get_e_machine<32, false>(elf_header);
          }
      }
      else if (size == 64){
          if (big_endian) {
              elfcpp::Ehdr<64, true> elf_header = ehdr;
              e_machine = get_e_machine<64, true>(elf_header);
          }
          else {
              elfcpp::Ehdr<64, false> elf_header = ehdr;
              e_machine = get_e_machine<64, false>(elf_header);
          }
      }
      /// xie: 基于e_machine确定架构类型.  这相当于读取每个文件时我们都确定了一遍
      if (e_machine == elfcpp::EM_AARCH64)
          layout_->shuffle_info_.arch = 2;
      else if (e_machine == elfcpp::EM_X86_64)
          layout_->shuffle_info_.arch = 1;
