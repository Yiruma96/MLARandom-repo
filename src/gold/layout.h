...
{
 public:
    /// xie: 将元数据存在layout这个全局变量里
    struct Metadata{
        ShuffleInfo::Metadata global_metadata;
        unsigned int arch = -1; // arch=1 x64架构   arch=2  aarch64架构
    } shuffle_info_;
    ...
    uint64_t c_fun = 0;
    uint64_t all_obj = 0;
    uint64_t hybrid_obj = 0;
    uint64_t static_got = 0;
    uint64_t silicon_errata = 0;
    uint64_t tls_opt = 0;
    uint64_t got_opt = 0;
    uint64_t got_target = 0;
    uint64_t tls_got_target = 0;
    uint64_t plt_target = 0;
    uint64_t merged_target = 0;
    uint64_t ifunc_target = 0;
    std::unordered_set<const char*> missrandfile;

    bool object_has_rand = false; /// 记录参与本次链接的file，是否有至少一个含有rand节的，该变量在relocatable模式下判断是否本次要收集元数据
    bool staticDynTLS = false; /// static+存在动态转静态优化下，需要在打开TLS优化的同时关闭元数据收集工作
    bool need_output_rand = false; /// 记录本次链接是否需要输出元数据，用于指导rand节建立、重定位收集、写元数据等操作
    Output_section* rand = NULL;

    /// xie: =============开始================

    /// 之前是白名单fixup收集，现在是黑名单fixup收集
    bool is_special_section_for_fixup(const char* secname){
        static const char* const special_fixup_section[] =
        {
            ".debug",
            ".eh_frame",
            ".gcc_except_table"
        ...
        for (size_t i = 0; i < sizeof(special_fixup_section) / sizeof(special_fixup_section[0]); i++)
            if (is_prefix_of(special_fixup_section[i], secname))
                return true;
        return false;
    }

    /// layout的收集范围和之前保持不变。因为实验中发现将随机化范围扩展到unlikely节的话，crt1.o的unlikely位于中间却没有相应的rand，
    /// 由于已经弃用了Hybrid-grained Randomization机制，我们不允许这种情况存在。
    bool is_special_section_for_layout(const char* secname){
    ...
    /// xie: 有些重定位实在少见，我们既难以constant中没有定义他们的处理规则，在reloc函数中也难以分析其计算过程，所以不记录，不修复吧
    bool black_reloc_type_ARM64(unsigned int r_type) {
        static unsigned int black_reloc_type_table_ARM64[] =
        {
                567, // R_AARCH64_TLSDESC_LDR
                568, // R_AARCH64_TLSDESC_ADD
                569  // R_AARCH64_TLSDESC_CALL
        };
        for (size_t i = 0; i < sizeof(black_reloc_type_table_ARM64) / sizeof(black_reloc_type_table_ARM64[0]); i++)
            if (r_type == black_reloc_type_table_ARM64[i])
                return true;
        return false;
    }

    template<int size, bool big_endian> ShuffleInfo::Metadata*
    extract_metadata(Sized_relobj_file<size, big_endian>* sized_relobj);

    template<int size, bool big_endian> void
    merge_layout(ShuffleInfo::Metadata* metadata,
                 Sized_relobj_file<size, big_endian> *containing_object,
                 unsigned int input_section_idx);

    template<int size, bool big_endian> void
    merge_fixup(ShuffleInfo::Metadata* sub_metadata,
                Sized_relobj_file<size, big_endian> *pfrom_object,
                Symbol_table *symtab);

    template<int size, bool big_endian> void
    set_fixup(ShuffleInfo::Metadata_FixupInfo* add_fixup,
              const ShuffleInfo::Metadata_FixupInfo &sub_fixup,
              Sized_relobj_file<size, big_endian> *pfrom_object, Symbol_table *symtab);


    template<int size, bool big_endian> void
    update_shuffleInfo(Symbol_table* symtab, const Input_objects* input_objects);

    template<int size, bool big_endian>
    void is_need_output_rand();

    ShuffleInfo::Metadata& get_global_metadata()
    {return this->shuffle_info_.global_metadata;}

    void add_other_fixup(uint64_t fixup_offset, Output_section *os,
    ...
    Layout(int number_of_input_files, Script_options*, std::string cmds); /// xie. add argument cmds
