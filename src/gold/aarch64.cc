...
/*
  # 勘误前
  6bcffc:	f944f800 	ldr	x0, [x0, #2544]
  6bd000:	b9000001 	str	w1, [x0]                 # 被勘误指令erratum_insn
  6bd004:	f94043e0 	ldr	x0, [sp, #128]

  # 勘误后
  6bcffc:	f944f800 	ldr	x0, [x0, #2544]
  6bd000:	141fe59b 	b	eb666c <__libc_csu_fini+0xc>   # branch-to-stub
  6bd004:	f94043e0 	ldr	x0, [sp, #128]

  0000000000eb6660 <__libc_csu_fini>:
    eb6660:	d65f03c0 	ret
    eb6664:	f9406c00 	ldr	x0, [x0, #216]    # erratum stub INSN#1
    eb6668:	17d32a69 	b	38100c <__module_alloc_space_7_MOD_alloc_space_field_core_7+0x48aec> # erratum stub INSN#2
* */
      while (p != end)
	{
	  The_erratum_stub* stub = *p;
      ...
      // xie: 将erratum insn替换为branch-to-stub指令。此时产生一个重定位
	  if (!try_fix_erratum_843419_optimized(stub, view_offset, pview))
		...
		stub_table->erratum_stub_address(stub);
	      ...
	      /// xie: 目前我修改了do_relocate_sections函数的芯片勘误入口来暂时禁用所有芯片勘误操作，其对于反汇编器评估不构成挑战因此没有意义
          /// xie: 没有optimized时，芯片勘误就会产生两条b指令，即两个重定位
//        Output_section* os = out_sections[i];
//        Layout* my_layout = os->get_layout();
//        my_layout->silicon_errata += 2;
//
//	      unsigned int fixup_offset = stub->erratum_address();
//	      my_layout->add_other_fixup(fixup_offset, os->name(), stub_address, fixup_offset, 0, 282);
//
//	      // 如果该地址存在fixup则在保存后替换
//	      ShuffleInfo::ReorderInfo_FixupInfo_FixupTuple* fixup_tuple = nullptr;
//	      fixup_tuple = stub->relobj()->fixup_text_map.at(fixup_offset);
		...
		b_offset & 0xfffffff);
          ...
          // executed.
          // xie: 修复erratum stub部分，包括替换后的指令以及一条b指令，又产生一个重定位
...
{
    /// xie: 在暂未搞清楚该优化的patch方案前，暂时禁用该优化
    return false;
...
class AArch64_output_section : public Output_section
{
 ...
 public:
  AArch64_output_section(const char* name, elfcpp::Elf_Word type,
			 elfcpp::Elf_Xword flags, Layout* my_layout)
    : Output_section(name, type, flags, my_layout)
  { }
...
class Target_aarch64 : public Sized_target<size, big_endian>
{
 ...
 protected:
  ...
  do_make_output_section(const char* name, elfcpp::Elf_Word type,
			 elfcpp::Elf_Xword flags, Layout* my_layout)
  { return new The_aarch64_output_section(name, type, flags, my_layout); }
 ...
 private:
  ...
  optimize_tls_reloc(bool is_final, int r_type, Layout* layout);
                    ...
                    section_size_type)
{
  ...
  switch(stub->type())
	...
	The_reloc_functions_status status =
	    The_reloc_functions::adrp(view, dest, 0, address);
    ...
    Address plt_address)
{
  ...
  AArch64_relocate_functions<size, big_endian>::adrp(pov + 4,
      gotplt_2nd_ent, 0, plt_address + 4);
...
/*
所以一个操作数到底是有符号数还是无符号数，最直观的就是看它的地址是绝对地址还是相对地址，因为相对地址的话一定要设计为有符号数，绝对地址的话一定要设计为无符号数
 那么反汇编器怎么判断呢？操作数是有符号还是无符号是可以从opcode中得到的，所以有这个opcode_tbl就很方便。
 按道理说这两者应该是相同的，但是比如说ldr指令，它既有相对寻址也有绝对寻址，那么就应该是opcode_tbl中有多条ldr，然后操作数有的是有符号的有的是无符号的，但我目前没有找到12bit的有符号数，很奇怪

 从aarch64-reloc.def中的重定位信息，以及aarch64.cc中的写入信息，这是一个正向的过程，但因为难以看出来操作数的正负，
 因此我从aarch64-tbl.h中找到指令和操作数的信息，来看指令的操作数为有符号还是无符号，这是一个逆向的过程
 但遗憾的是，两者得出的结论并不统一，最会决定还是按照写入方式，因为ldr类的指令一定就是按照ldr的howto来写的，我管你后面是怎么解析的！


 Y(IMMEDIATE, imm_half, "HALF", 0, F(FLD_imm16), "a 16-bit immediate with optional left shift")
 ...
 Y(ADDRESS, addr_simple, "SIMD_ADDR_SIMPLE", 0, F(), "an address with base register (no offset)")
 ...
 Y(ADDRESS, imm, "ADDR_PCREL21", OPD_F_SEXT, F(FLD_immhi,FLD_immlo), "21-bit PC-relative address")
 CORE_INSN ("adr",  0x10000000, 0x9f000000, pcreladdr, OP2 (Rd, ADDR_PCREL21), QL_ADRP, 0),

 X(ADDRESS, 0, ext_imm, "ADDR_ADRP", OPD_F_SEXT, F(FLD_immhi, FLD_immlo), "21-bit PC-relative address of a 4KB page")
 CORE_INSN ("adrp", 0x90000000, 0x9f000000, pcreladdr, OP2 (Rd, ADDR_ADRP), QL_ADRP, 0),

 Y(IMMEDIATE, aimm, "AIMM", 0, F(FLD_shift,FLD_imm12), "a 12-bit unsigned immediate with optional left shift of 12 bits")\
 { "add",  0x11000000, 0x7f000000, addsub_imm, OP_ADD, CORE, OP3 (Rd_SP, Rn_SP, AIMM), QL_R2NIL, F_HAS_ALIAS | F_SF, NULL},



 Y(ADDRESS, addr_uimm12, "ADDR_UIMM12", 0, F(FLD_Rn,FLD_imm12),	 "an address with scaled, unsigned immediate offset")
  Y(ADDRESS, addr_simm, "ADDR_SIMM9", 0, F(FLD_imm9,FLD_index),	 "an address with 9-bit signed immediate offset")
 ...
 Y(ADDRESS, imm, "ADDR_PCREL14", OPD_F_SEXT | OPD_F_SHIFT_BY_2,	F(FLD_imm14), "14-bit PC-relative address")
  ...
  Y(ADDRESS, imm, "ADDR_PCREL19", OPD_F_SEXT | OPD_F_SHIFT_BY_2,  F(FLD_imm19), "19-bit PC-relative address")
...
class AArch64_relocate_functions
{
 ...
 public:
  ...
  /// x=psymval+add target=psymval writeval
  template<int valsize>
  static inline typename This::Status
  rela_ua(unsigned char* view,
	  ...
	  const AArch64_reloc_property* reloc_property)
  {
      typedef typename elfcpp::Swap_unaligned<valsize, big_endian>::Valtype Valtype;

      /// xie:
      if (need_update || need_add) {
          std::pair<AArch64_valtype, Addendtype> temp = psymval->template getValueAddend(object, addend);
          target_address = temp.first;
          target_add = temp.second;
          has_record = true;
      }
...
/// x=psymval+add-address target=psymval writeval
  template<int valsize>
  static inline typename This::Status
...
/// x=psymval+add target=psymval reloc_common
  template<int valsize>
  static inline typename This::Status
...
/// x=s+add target=s reloc_common
  ...
  // new val = (val & ~dst_mask) | (immed << doffset)
/// x=psymval+add-address target=psymval reloc_common
  template<int valsize>
  static inline typename This::Status
  pcrela_general(
    unsigned char* view,
...
/// x=psymval+add-address target=psymval update_adr
  static inline typename This::Status
      ...
      const AArch64_reloc_property* /* reloc_property */)
  {
      ...
      /// xie:
      if (need_update || need_add) {
          std::pair<Address, Addendtype> temp = psymval->template getValueAddend(object, addend);
          target_address = temp.first;
          target_add = temp.second;
          base_address = address;
          has_record = true;
  ...
  // R_AARCH64_ADR_PREL_PG_HI21
/// x=Page(s+a)-Page(address) target=sa update_adr
  static inline typename This::Status
  adrp(
    unsigned char* view,
    Address s,
    ...
    Address address)
  {
      ...
      if (need_update || need_add) {
          target_address = s;
          target_add = (Addendtype)add;
          base_address = address;
          has_record = true;
      }

    AArch64_valtype x = This::Page(s+add) - This::Page(address);
    // Pick [32:12] of X.
  ...
  // Calculate PG(S+A) - PG(address), update adrp instruction.
  // R_AARCH64_ADR_PREL_PG_HI21
/// x=Page(psmval+add)-Page(address) target=psmval update_adr
  static inline typename This::Status
						     ...
						     int r_type, Layout* layout)
{
  ...
  /// xie: 非shared文件下gold会进行tls优化，判断是否可以关闭该优化
  tls::Tls_optimization op_retu;
  bool dynamic2exec = false;
 ...
 switch (r_type)
 {
     ...
     case elfcpp::R_AARCH64_TLSDESC_CALL:
         ...
         if (is_final)
             op_retu = tls::TLSOPT_TO_LE;
         else
             op_retu = tls::TLSOPT_TO_IE;
         dynamic2exec = true;
         break;
 ...
 /// xie: 静态链接 && 动态转静态下，TLS优化是必须要的，此时允许TLS优化，但同时通知取消元数据合并
 if (parameters->doing_static_link() && dynamic2exec){
     if (layout != NULL)
         layout->staticDynTLS = true;
 }
 /// xie: 其他情况下，为了保证元数据收集的正确性，我们关闭TLS优化
 else
     return tls::TLSOPT_NONE;
 return op_retu;
}
    ...
    bool is_discarded)
{
  ...
  switch (r_type)
	...
	tls::Tls_optimization tlsopt = Target_aarch64<size, big_endian>::
	  optimize_tls_reloc(!parameters->options().shared(), r_type, layout);
    ...
    Symbol* gsym)
{
  ...
  switch (r_type)
    ...
    case elfcpp::R_AARCH64_TLSGD_ADD_LO12_NC:  // General dynamic
      {
	tls::Tls_optimization tlsopt = Target_aarch64<size, big_endian>::
	    optimize_tls_reloc(gsym->final_value_is_known(), r_type, layout);
	if (tlsopt == tls::TLSOPT_TO_LE)
	  {
	    layout->set_has_static_tls();
	    break;
...
/*
(arm版本)关于什么时候需要重新need_update的结论如下：
arm的难点在于：一方面由于组合重定位的存在，需要重新收集以明确got和tls类型的base和target；另一方面重新收集又会破坏编译时收集的base和target。

我们现在将重定位范围限制在了GOT和TLS类型的重定位中，但还要考虑这些重定位中是否仍然存在target为normal_sym或是base为sym的情况，也就是做进一步过滤
- 类似R_X86_64_GOTOFF64，其target为normal。
    arm版本中只有R_AARCH64_GOTREL64和R_AARCH64_GOTREL32是这种情况，看起来他们不需要组合重定位，因此直接过滤这两个重定位即可
- 类似R_X86_64_GOTPC64，其base为SYM。直接过滤这些基址为PC的GOT和TLS重定位即可。
可以看到还好，这些被过滤掉的都不涉及组合重定位因此无需精确收集
...
Target_aarch64<size, big_endian>::Relocate::relocate(
    ...
    /// xie: 通过观察AArch64_relocate_functions中最底层的写值函数，看到target是可以在那里捕获到的。向上追溯target的形成过程，也可以看到target并非都是从psyval中得到的。
    /// 这说明我们对target地址的收集仍不准确。那么就需要在gas->gold的merge_fixup之外，在处理重定位的时候，针对以下三类情况更新其target：
    /// 1. plt函数，ifunc，merge这三种情况，属于psymval机制但又需要适当变化的。
    /// 2. got_entry plt_entry 他们的target和base非psymval机制，所以最好从重定位函数中观察
    /// 3. 重定位中新增的fixup
    /// 除了1外，2和3都是在重定位函数中准确收集target_addr和base_addr的

    /// 从源码中收集指针寻址模式的难点：
    /// 1. 从gas中提取最初的base，target，add
    /// 2. 部分指针的base，target会发生变化；部分指针的符号无法确定，需要在链接器中解决
    /// 3. 在链接阶段，一方面我们需要知道拿到这些符号对应的地址，
    ///     另一方面指针的target可能发生变化因此需要更新，比如merged_fixup, plt, ifunc, 这时候我们替换掉target(这三种情况gas中应该不会有base和target的改变因此只有一套式子)，此外merged_fixup的add应该置为0
    ///     got plt两类更是直接抛弃gas中收录的结果，直接应用重定位中的情况，这时候应用target,base,add即可(是否可能导致sym变pc，target变section？)

    /// 讨论：更新时使用重定位中的target base add是否会存在target和baes不准确的情况？
    /// 目前遇到的add有三种情况
    /// 1. 第一种是作为内存对象中的偏移
    /// 2. 第二种是作为基址为pc时候的偏移
    /// 3. 第三种是作为target和base变化后的一个偏移
    /// 第三种说明target产生了降级，因此只要不是第三种就行，而我想got和plt类型的应该不会这样的，也就是应该不会在gas写重定位时变化符号

    /// xie:
    auto *my_layout = const_cast<Layout *>(relinfo->layout);
    ShuffleInfo::Metadata_FixupInfo* fixup_tuple = nullptr;
    need_output_rand = my_layout->need_output_rand;
    need_update = false;
    need_add = false;
    has_record = false;
    target_address = 0;
    ...
    /// xie:
    int fixup_offset = static_cast<int>(address) - os->address();

    /// 这里会遍历每个重定位项，在开启随机化开关时，给其对应的fixup打上重定位的标记
    if (my_layout->need_output_rand && relinfo->object->has_rand){
        auto find_retu = os->fixup_map.find(fixup_offset);
        if (find_retu == os->fixup_map.end())
            fixup_tuple = nullptr;
        else
            fixup_tuple = find_retu->second;

        if (fixup_tuple != nullptr){
            /// 在第9个bit上打上1的标记以表明该fixup属于重定位项
            fixup_tuple->set_type( fixup_tuple->type() | 256);
        }
    }

    /// 三处fixup target地址的修正. 当输出结果含有元数据时(need_output_rand)，修改fixup的值
    bool update_psymval = false;
    Symbol_value<size> symval;
    /// target地址由于plt符号调整发生了改变
    if (gsym != NULL && gsym->use_plt_offset(reloc_property->reference_flags())) {
        symval.set_output_value(target->plt_address_for_global(gsym));
        psymval = &symval;

        update_psymval = true;
        target_address = psymval->input_value();
    }
    /// target地址由于ifunc符号调整发生了改变
    ...
    /// target地址由于符号位于SHF_MERGE节，当时使用的psymval->input_value()是错误的，只有此时配合上重定位项里的add才可以计算
    else if (!psymval->has_output_value()) {
        target_address = psymval->value(object, rela.get_r_addend());
        update_psymval = true;
    }

    /// xie: 更新第一波fixup 由于target_sym发生了改变
    if ( need_output_rand && update_psymval && fixup_tuple != nullptr) {
        fixup_tuple->set_target_bbl_sym(target_address);
        if (!psymval->has_output_value()) // 如果是merged的话还需要置空add
            fixup_tuple->set_add(0);
    }

    /// xie: 新增第一波fixup？如果输出元数据 & 该obj没有rand节 & 非几个特殊的节
    // 之前是白名单fixup收集，现在是黑名单fixup收集
    need_add = need_output_rand && !relinfo->object->has_rand && !my_layout->is_special_section_for_fixup(os->name());
    ...
    /// xie: 更新第二波fixup 主要是更新下tls和got的
    if (need_output_rand && need_update && fixup_tuple != nullptr && !my_layout->black_reloc_type_ARM64(r_type)){
        if (has_record){
            fixup_tuple->set_target_bbl_sym(target_address);
            fixup_tuple->set_add(target_add);

            unsigned int _type = fixup_tuple->type();
            if ((_type & 0x3) != 0) // 如果基址没有记录的话则无需更新，因为当时没有记录意味着base并非符号。是pc或是none，这两者不会改变因此也无需进一步的精确记录
                fixup_tuple->set_base_bbl_sym(base_address);
        }else{
            printf("Fixup (r_type=%d)target位于plt或got中，但没有在重定位处理函数中找到它们的值\n%s", r_type, my_layout->my_cmd.c_str());
            exit(0x233);
        }
    }

    /// xie: 加入第一波的fixup
    if (need_output_rand && need_add && !my_layout->black_reloc_type_ARM64(r_type)) {
        if (has_record){
            /// 收集自重定位函数，因为有一些使用psymval(obj, add)拿到值，所以target里是包含add的，所以add参数置为固定的0
            my_layout->add_other_fixup(fixup_offset, os, target_address, base_address, target_add, r_type, 1);
        }else{
            printf("该重定位 (obj=%s rel_num=%ld reloc_type=%u output_sec=%s)所在obj没有rand节因此需要新增，但没有在重定位处理函数中收集到它的信息，请排查是否该fixup所处的section需要加入黑名单\n",
                   relinfo->object->name().c_str(), relnum, r_type, os->name());
            exit(0x233);
        }
    }
    ...
    typename elfcpp::Elf_types<size>::Elf_Addr address)
{
  ...
  /// xie: tls符号的offset是否能够确定
  const bool is_final = (gsym == NULL
			 ? !parameters->options().shared()
			 : gsym->final_value_is_known());
  /// xie: 根据is_final和is_exe，对tls的四种类型做一个优化
  tls::Tls_optimization tlsopt = Target_aarch64<size, big_endian>::
      optimize_tls_reloc(is_final, r_type, NULL);
	     ...
	     const Symbol_value<size>* psymval)
{
  ...
  if (r_type == elfcpp::R_AARCH64_TLSGD_ADD_LO12_NC)
    {
      ...
      // done.
      /// xie: 两个新的重定位都已经在处理第一个时处理完了，因此第二个不做处理
  ...
  /// xie: 为写入的第二条指令处理重定位R_AARCH64_TLSLE_ADD_TPREL_HI12
  ...
  /// xie: 为写入的第三条指令处理重定位R_AARCH64_TLSLE_ADD_TPREL_LO12
...
/// xie: 接下来的这三个优化都是指令对指令的，所以使用switch结构来处理
template<int size, bool big_endian>
inline
typename AArch64_relocate_functions<size, big_endian>::Status
	     ...
	     const Symbol_value<size>* psymval)
{
  ...
  if (r_type == elfcpp::R_AARCH64_TLSIE_ADR_GOTTPREL_PAGE21)
    { /// xie: R_AARCH64_TLSIE_ADR_GOTTPREL_PAGE21转为R_AARCH64_TLSLE_MOVW_TPREL_G1
    ...
    { /// xie: R_AARCH64_TLSIE_ADR_GOTTPREL_PAGE21转为R_AARCH64_TLSLE_MOVW_TPREL_G0_NC
	     ...
	     const Symbol_value<size>* psymval)
{
  ...
  switch (r_type)
    ...
    case elfcpp::R_AARCH64_TLSDESC_ADD_LO12:
    case elfcpp::R_AARCH64_TLSDESC_CALL:
        /// xie: R_AARCH64_TLSDESC_CALL和R_AARCH64_TLSDESC_ADD_LO12删除
    ...
    case elfcpp::R_AARCH64_TLSDESC_ADR_PAGE21:
        /// xie: R_AARCH64_TLSDESC_ADR_PAGE21转为R_AARCH64_TLSLE_MOVW_TPREL_G1
    ...
    case elfcpp::R_AARCH64_TLSDESC_LD64_LO12:
        /// xie: R_AARCH64_TLSDESC_LD64_LO12转为R_AARCH64_TLSLE_MOVW_TPREL_G0_NC
	     ...
	     typename elfcpp::Elf_types<size>::Elf_Addr address)
{
  ...
  switch (r_type)
    ...
    case elfcpp::R_AARCH64_TLSDESC_ADR_PAGE21:
      {
          /// xie: R_AARCH64_TLSDESC_ADR_PAGE21保持不变
	return aarch64_reloc_funcs::adrp(view, got_entry_address, addend,
					 address);
      }
      break;
	...
	/// xie: R_AARCH64_TLSDESC_LD64_LO12转为R_AARCH64_TLSIE_LD64_GOTTPREL_LO12_NC类型
