...
class Relobj : public Object
{
 public:
  ...
  /// xie: =========开始==========
    ...
    const Output_sections& output_sections() const
    { return this->output_sections_; }

    Output_section* get_output_section(unsigned int shndx){
        return this->output_sections_[shndx];
    }


    /// xie: layout虽然是逐input_section添加，但fixup不能每次都添加。
    bool is_fixup_has_added(){
        return this->fixup_has_added;
    }
    void set_fixup_has_added(){
        this->fixup_has_added = true;
    }
    bool fixup_has_added = false;
 ...
 protected:
  ...
  // Set the size of the relocatable relocs array.output_sections
  void
...
class Symbol_value
{
 public:
  ...
  /// xie:
    ...
    getValueAddend(const Sized_relobj_file<size, big_endian>* object, Value addend) const
    {
        if (this->has_output_value_)
            return std::pair<Value, Value>(this->u_.value, addend);
        else
        {
            gold_assert(this->is_ordinary_shndx_);
            return std::pair<Value, Value>(this->u_.merged_symbol_value->value(object, this->input_shndx_, addend), 0);
        }
    }
...
class Sized_relobj_file : public Sized_relobj<size, big_endian>
{
 public:
    /// xie:
    std::map<uint32_t, bool> shndx_map;  /// 记录obj中的下标和节名对应
    unsigned rand_ndx = 0;               /// 记录rand节的下标
    bool has_rand = false;
    bool has_parse_protobuf = false;
    ShuffleInfo::Metadata metadata;
    section_size_type rand_sec_len;
    bool obj_has_count = false;
  ...
  { return this->is_deferred_layout_; }
    ...
    /// xie: 不考虑同名节的情况
    void record_shndx(uint32_t ndx){
        this->shndx_map[ndx] = true;
    }

    bool section_exist(uint32_t ndx){
        return this->shndx_map.find(ndx) != this->shndx_map.end();
    }

    void set_fixup(ShuffleInfo::Metadata_FixupInfo *add_fixup,
...
protected:
 ...
 private:
  ...
  write_sections(const Layout*, const unsigned char* pshdrs, Output_file*,
		 Views*, const Symbol_table* symtab);
