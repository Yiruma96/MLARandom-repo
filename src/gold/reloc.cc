...
Sized_relobj_file<size, big_endian>::do_read_relocs(Read_relocs_data* rd)
{
  ...
  /// 遍历obj中的所有节,并挑出来重定位节,进入到relocate_section函数中处理
						 ...
						 Output_file* of)
{
  ...
  this->write_sections(layout, pshdrs, of, &views, symtab);
...
template<int size, bool big_endian>
void Sized_relobj_file<size, big_endian>::set_fixup(ShuffleInfo::Metadata_FixupInfo *add_fixup,
                           const ShuffleInfo::Metadata_FixupInfo &sub_fixup,
                           const Symbol_table *symtab,
                           const std::vector<Output_section*>& out_sections,
                           const std::vector<Address>& out_offsets) {

    uint32_t sec_idx = sub_fixup.section();

    add_fixup->set_section(out_sections[sec_idx]->do_out_shndx());
    add_fixup->set_offset(out_offsets[sec_idx] + sub_fixup.offset());
    add_fixup->set_add(sub_fixup.add());
    if (sub_fixup.has_step())
        add_fixup->set_step(sub_fixup.step());
    add_fixup->set_type(sub_fixup.type());

    unsigned base_type = sub_fixup.type() & 3;
    if (base_type == 3) { // 存储INDEX的情况下，需要将符号表下标替换为新的，注意这建立在符号一定会继承下去的假设之下
        uint64_t base_symidx = sub_fixup.base_bbl_sym();
        uint64_t new_base_symidx;
        if (base_symidx < this->local_symbol_count_){ // 局部符号
            new_base_symidx = this->symtab_index(base_symidx);
        } else{ // 全局符号
            const Symbol* gsym = this->global_symbol(base_symidx);
            if (gsym->is_forwarder())
                gsym = symtab->resolve_forwards(gsym);
            new_base_symidx = gsym->symtab_index();
        }
        if (new_base_symidx == -1U)
            gold_error("在--relocatable时，fixup的base符号不会继承到输出，需修改为value形式");
        add_fixup->set_base_bbl_sym(new_base_symidx);
    } else if (base_type == 2) { // 存储VALUE的情况下，需要更新base_section和base_bbl_sym
        uint32_t base_section_index = sub_fixup.base_section();
        add_fixup->set_base_section(out_sections[base_section_index]->do_out_shndx());
        add_fixup->set_base_bbl_sym(out_offsets[base_section_index] + sub_fixup.base_bbl_sym());
    }

    unsigned target_type = (sub_fixup.type() >> 2) & 3;
    if (target_type == 3) { // 存储INDEX的情况下，需要将符号表下标替换为新的，注意这建立在符号一定会继承下去的假设之下
        uint64_t target_symidx = sub_fixup.target_bbl_sym();
        uint64_t new_target_symidx;
        if (target_symidx < this->local_symbol_count_){ // 局部符号
            new_target_symidx = this->symtab_index(target_symidx);
        } else{ // 全局符号
            const Symbol* gsym = this->global_symbol(target_symidx);
            if (gsym->is_forwarder())
                gsym = symtab->resolve_forwards(gsym);
            new_target_symidx = gsym->symtab_index();
        }
        if (new_target_symidx == -1U)
            gold_error("在--relocatable时，fixup的target符号不会继承到输出，需修改为value形式");
        add_fixup->set_target_bbl_sym(new_target_symidx);
    } else if (target_type == 2){ // 存储VALUE的情况下，需要更新target_section和target_bbl_sym
        uint32_t target_section_index = sub_fixup.target_section();
        add_fixup->set_target_section(out_sections[target_section_index]->do_out_shndx());
        add_fixup->set_target_bbl_sym(out_offsets[target_section_index] + sub_fixup.target_bbl_sym());
    }
                            ...
                            const Symbol_table* symtab)
{
  ...
  bool is_sorted = true;
    ...
    /// xie: --relocatable时，我们选择在该函数内合并元数据，因为该函数会被每一个obj执行到。注意该实现并没有经过验证！！！！！
    /// 目前的实现是基于可迭代思想的，也就是relocate后的obj可以再次relocate。因为gold不会将offset改为VA，只是将input_section的offset改成output_section的offset，同时更新section的idx
    Layout* my_layout = const_cast<Layout*>(layout);
    ShuffleInfo::Metadata sub_metadata;
    ShuffleInfo::Metadata &global_metadata = my_layout->get_global_metadata();

    /// xie: 每个obj只序列化一次
    if (my_layout->need_output_rand && parameters->options().relocatable() && this->has_rand){
        section_size_type rand_sec_len;
        const unsigned char *rand_contents = this->section_contents(this->rand_ndx, &rand_sec_len, false);
        std::string rand_input(reinterpret_cast<const char *>(rand_contents), rand_sec_len);
        sub_metadata.ParseFromString(rand_input);
    }

    /// xie: 首先合并fixup元数据，这个是每个obj只合并一次: 注意我们只是合并，不需要从重定位中补全，该工作可以等到非--relocatable时再做补全
    if (my_layout->need_output_rand && parameters->options().relocatable() && this->has_rand) {
        ShuffleInfo::Metadata_FixupInfo *add_fixup = NULL;

        for (int i = 0; i < sub_metadata.fixups_size(); ++i) {
            const ShuffleInfo::Metadata_FixupInfo& sub_fixup = sub_metadata.fixups(i);

            if (!this->section_exist(sub_fixup.section()))
                continue;

            add_fixup = global_metadata.add_fixups();
            set_fixup(add_fixup, sub_fixup, symtab, out_sections, out_offsets);
        }
  ...
  /// 该循环遍历该子obj的所有节，并记录到rm以写入可执行文件中。我们识别加入到.text中的input_section，也就是目前只合并他们的元数据。
  /// 注意这是目前唯一非迭代式的实现，因为真正的obj file的layout可能是.text.xxx，而relocate合并出的obj file的layout只能是.text
  for (unsigned int i = 1; i < shnum; ++i, p += This::shdr_size)
	...
	continue;
      ...
      /// xie: --relocatable下合并layout元数据————需要收录所有os为.text节的
      if (my_layout->need_output_rand && !strcmp(".text", os->name()) && parameters->options().relocatable()){

          ShuffleInfo::Metadata_FunInfo* add_fun;
          ShuffleInfo::Metadata_InstInfo* add_inst;

          for (int j=0; j < sub_metadata.funs_size(); j++){
              const ShuffleInfo::Metadata_FunInfo& sub_fun = sub_metadata.funs(j);
              /// 可能该fun所在的section并非本轮的input_section的idx，跳过
              if (sub_fun.section() != i)
                  continue;

              /// fun信息迁移
              add_fun = global_metadata.add_funs();
              add_fun->set_offset(this->section_offsets()[sub_fun.section()] + sub_fun.offset());
              Output_section* output_section = this->get_output_section(sub_fun.section());
              add_fun->set_section(output_section->out_shndx());
              add_fun->set_info(sub_fun.info());

              for (int k=0; k< sub_fun.insts_size(); k++){
                  const ShuffleInfo::Metadata_InstInfo& sub_inst = sub_fun.insts(k);

                  /// inst信息迁移
                  add_inst = add_fun->add_insts();
                  add_inst->set_offset(this->section_offsets()[sub_fun.section()] + sub_inst.offset());
                  add_inst->set_size(sub_inst.offset());
              }
          }
      }
    ...
    unsigned int end_shndx)
{
  ...
  for (unsigned int i = start_shndx; i <= end_shndx; ++i, p += This::shdr_size)
	...
	rr = this->relocatable_relocs(i);
      ...
      /// - 如果使用--relocatable参数，则不relocate_section，直接relocate_relocs记录重定位
      /// - 如果使用--emit-relocs参数，则relocate_section的同时，还要relocate_relocs记录重定位
      /// - 如果不使用上面两个参数，则只relocate_section
      if (!parameters->options().relocatable())
	{
      /// 在该函数中处理该重定位节
