 ...
 private:
  ...
  class Got_entry
  {
   public:
    ...
    void
    write(unsigned int got_indx, unsigned char* pov, Layout* my_layout, Output_section* os, uint64_t got_offset) const;
...
class Output_section : public Output_data
{
 public:
  ...
  Output_section(const char* name, elfcpp::Elf_Word, elfcpp::Elf_Xword, Layout* my_layout=NULL);
  virtual ~Output_section();
  ...
  /// xie: output_section中保存layout
  Layout* get_layout()
  {return this->my_layout;}
  /// xie: fixup_map从每个obj的五六个map，转为每个output_section一个。因为fixup来自不确定数量的节，而map以offset作为key会导致冲突
  std::unordered_map<uint64_t, ShuffleInfo::Metadata_FixupInfo*> fixup_map;
...
//        gold_info("For a merge section. ");
    ...
    {
        /// xie:
//        gold_info("For a relaxed input section. ");
	...
	this->u2_.posd->print_merge_stats(section_name);
      ...
      /// xie: input_section的名字
      std::string *insname_ = NULL;

      void record_input_section_name(const char * inname)
      {this->insname_ = new std::string(inname);}

      const char* get_input_section_name(){
        if (this->insname_)
  ...
  /// xie: 修改protected为public，以得知该output_section的下标
...
protected:
 ...
 private:
  ...
  /// xie: output_section记录全局变量layout
