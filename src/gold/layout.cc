...
Layout::Layout(int number_of_input_files, Script_options* script_options, std::string cmds)
...
Layout::layout(Sized_relobj_file<size, big_endian> *object, unsigned int shndx,
    ...
    *off = 0;
    ...
    /// xie: 输入InputSection的name和shndx，返回对应的OutputSection。这说明该子节是加入的
    object->record_shndx(shndx);

    if (!strcmp(name, ".rand")) {
        object_has_rand = true;
        object->has_rand = true;
        object->rand_ndx = shndx;
        return NULL;
    }
			    ...
			    Output_section_order order, bool is_relro)
{
  ...
  if ((flags & elfcpp::SHF_ALLOC) == 0
      ...
      if (type == elfcpp::SHT_PROGBITS)
	{
      ...
      os = target->make_output_section(name, type, flags, this);
    }
...
get_symbol_value(Sized_relobj_file<size, big_endian> *q_obj, Symbol_table *symtab, unsigned sym_index) {
    if (sym_index < q_obj->local_symbol_count()) {
        // 局部符号使用symbol_value来存储，也许是因为没有进入符号表的必要
        const Symbol_value<size> *psymval = q_obj->local_symbol(sym_index);
        return psymval->input_value(); // 不管是input_value还是output_value，都是拿里面的value值，这就够了
    } else { // local符号的后面就全是global符号了，使用symbol存储
        const Symbol *gsym = q_obj->global_symbol(sym_index);
        if (gsym == NULL)
            return 0;

        if (gsym->is_forwarder()) // symbol有可能是forwarder的，需要(Resolve the forwards from FROM, returning the real symbol.)
            gsym = symtab->resolve_forwards(gsym);

        // Symbol需要给出size，才能转化为sized_symbol形式，以得到符号的value
        const auto *sym = static_cast<const Sized_symbol<size> *>(gsym);
        return sym->value();
    }
}

/// xie: 判断本次链接的输出是否含有元数据，为是否创建rand节、是否收集重定位、是否写元数据提供依据
template<int size, bool big_endian>
void Layout::is_need_output_rand() {

    /// 默认关闭元数据收集
    this->need_output_rand = false;

    /// 判断元数据收集是否要开启：非staticDynTLS && 存在.rand节的obj
    if(!parameters->doing_static_link()) {
        ...
        for (Section_list::const_iterator p = this->section_list_.begin(); p != this->section_list_.end(); ++p) {
            ...
            Sized_relobj_file<size, big_endian> *q_obj;

            if (!strcmp(os->name(), ".text")) {

                for (auto q = input_section_list.begin(); q != input_section_list.end(); q++) {
                    if (!(q->is_input_section() && q->relobj()))
                        continue;
                    q_obj = reinterpret_cast<Sized_relobj_file<size, big_endian> *>(q->relobj());

                    if (!is_special_section_for_layout(q->insname_->c_str()) &&
                        q_obj->has_rand &&
                        q->data_size() != 0) {

                        this->need_output_rand = true;
                    }
                }
            }
        }
        if (!this->need_output_rand)
            gold_info("不进行随机化(非特殊节中没有含有rand的)");
    }else
        gold_info("不进行随机化(本次编译为static)");
}

/// 该函数运行后会将解析出的元数据缓存在rel_obj中
template<int size, bool big_endian>
ShuffleInfo::Metadata*
Layout::extract_metadata(Sized_relobj_file<size, big_endian>* sized_relobj) {
    ShuffleInfo::Metadata* metadata = NULL;
    section_size_type rand_sec_len;

    if (sized_relobj->has_rand) {
        if (!sized_relobj->has_parse_protobuf) {
            const unsigned char *rand_contents = sized_relobj->section_contents(sized_relobj->rand_ndx, &rand_sec_len, false);
            std::string rand_input(reinterpret_cast<const char *>(rand_contents), rand_sec_len);
            sized_relobj->metadata.ParseFromString(rand_input);
            sized_relobj->has_parse_protobuf = true;
        }
        metadata = &(sized_relobj->metadata);
        rand_sec_len = sized_relobj->rand_sec_len;
    }
...
template<int size, bool big_endian>
void
Layout::update_shuffleInfo(Symbol_table *symtab, const Input_objects* input_objects) {

    /// Step1. 遍历.text output section，合并子layout元数据
    for (auto p = this->section_list_.begin(); p != this->section_list_.end(); ++p) {

        /// Step1. layout元数据要按照text output section的顺序来拼接
        Output_section *os = *p;
        if (!strcmp(os->name(), ".text")) {

            Output_section::Input_section_list input_section_list = os->input_sections();
            Output_section::Input_section_list::const_iterator q_next;
            Sized_relobj_file<size, big_endian> *q_obj;
            Sized_relobj_file<size, big_endian> *q_next_obj;
            uint64_t off;
            uint64_t off_next;
            ...
            for (auto q = input_section_list.begin(); q != input_section_list.end(); q++) {
                ...
                /// 打印text节中所有的input_section
                if (parameters->options().print_layout()){
                    gold_info("%s obj=%s insname=%s shndx=%d off=%lx dataSize=%lx",
                              q_obj->has_rand? "***":"",
                              q->relobj()->name().c_str(),
                              q->insname_->c_str(),
                              q->shndx(),
                              q_obj->section_offsets()[q->shndx()],
                              q->data_size());
                }

                /// 静态链接的细节统计
                if (!q_obj->obj_has_count){
                    this->all_obj += 1;
                    if (!q_obj->has_rand) {
                        this->hybrid_obj += 1;
                        this->missrandfile.insert(q->relobj()->name().c_str());
                    }
                    q_obj->obj_has_count = true;
                }

                /// 提取元数据并开始拼接。注意我们不在gold中判断随机化范围，没有元数据的input_section就为它补充上一个
                ShuffleInfo::Metadata *metadata = extract_metadata(q_obj);
                if (metadata) {
                    merge_layout(metadata, q_obj, q->shndx());
                }else{
                    if (q->data_size() != 0) {
                        ShuffleInfo::Metadata &global_metadata = get_global_metadata();
                        ShuffleInfo::Metadata_FunInfo *add_fun = global_metadata.add_funs();
                        add_fun->set_info(str_concat("np-temp ", str_concat(str_concat(q->insname_->c_str(), " in "), q_obj->name().c_str())));
                        add_fun->set_offset(q_obj->section_offsets()[q->shndx()]);
                        add_fun->set_section(os->out_shndx());
                    }
                }
            }
        }
    ...
    /// Step2. 遍所有obj，做两件事情，一是合并子fixup元数据，二是累加gold_info.c_fun
    Sized_relobj_file<size, big_endian>* sized_relobj;
    for (auto relobj_iter = input_objects->relobj_begin(); relobj_iter != input_objects->relobj_end(); ++relobj_iter){

        sized_relobj = reinterpret_cast<Sized_relobj_file<size, big_endian> *>(*relobj_iter);
        if (!sized_relobj->is_fixup_has_added()) {

            /// 提取元数据
            ShuffleInfo::Metadata* metadata = extract_metadata(sized_relobj);

            if (metadata){
                /// 合并gold_info中的信息
                if (metadata->has_goldinfo())
                    this->c_fun += metadata->goldinfo().c_fun();

                /// 开始合并fixup元数据
                merge_fixup(metadata, sized_relobj, symtab);
            }
...
template<int size, bool big_endian>
void
Layout::merge_layout(ShuffleInfo::Metadata* sub_metadata,
                     Sized_relobj_file<size, big_endian> *pfrom_object,
                     unsigned int input_section_idx) {
    ShuffleInfo::Metadata &global_metadata = get_global_metadata();

    ShuffleInfo::Metadata_FunInfo* add_fun;
    ShuffleInfo::Metadata_InstInfo* add_inst;

    for (int i=0; i < sub_metadata->funs_size(); i++){
        const ShuffleInfo::Metadata_FunInfo& sub_fun = sub_metadata->funs(i);
        /// 可能该fun所在的section并非本轮的input_section的idx，跳过
        if (sub_fun.section() != input_section_idx)
            continue;

        /// fun信息迁移
        add_fun = global_metadata.add_funs();
        add_fun->set_offset(pfrom_object->section_offsets()[sub_fun.section()] + sub_fun.offset());
        Output_section* output_section = pfrom_object->get_output_section(sub_fun.section());
        add_fun->set_section(output_section->out_shndx()); /// 这个意义不大，因为我们只关注text节
        add_fun->set_info(sub_fun.info());

        for (int j=0; j < sub_fun.insts_size(); j++){
            const ShuffleInfo::Metadata_InstInfo& sub_inst = sub_fun.insts(j);

            /// inst信息迁移
            add_inst = add_fun->add_insts();
            add_inst->set_offset(pfrom_object->section_offsets()[sub_fun.section()] + sub_inst.offset());
            add_inst->set_size(sub_inst.size());
        }
    }
}


/// 将子rand(to_add_info)中的fixup添加到global_shuffledInfo中
template<int size, bool big_endian>
void
Layout::merge_fixup(ShuffleInfo::Metadata* sub_metadata,
                    Sized_relobj_file<size, big_endian> *pfrom_object,
                    Symbol_table *symtab) {

    ShuffleInfo::Metadata &global_metadata = get_global_metadata();

    ShuffleInfo::Metadata_FixupInfo* add_fixup;

    for(int i=0; i < sub_metadata->fixups_size(); i++){
        const ShuffleInfo::Metadata_FixupInfo& sub_fixup = sub_metadata->fixups(i);

        // 判断Fixup所在节是否要丢弃
        if (!pfrom_object->section_exist(sub_fixup.section()))
            continue;

        add_fixup = global_metadata.add_fixups();
        set_fixup(add_fixup, sub_fixup, pfrom_object, symtab);
    }
}


template<int size, bool big_endian>
void Layout::set_fixup(ShuffleInfo::Metadata_FixupInfo* add_fixup,
                      const ShuffleInfo::Metadata_FixupInfo &sub_fixup,
                      Sized_relobj_file<size, big_endian> *pfrom_object, Symbol_table *symtab) {

    typedef typename elfcpp::Elf_types<size>::Elf_Addr Address;
    const std::vector<Address> &out_offsets(pfrom_object->section_offsets()); /// 该obj的input_section在output_section中的offset
    Output_section* output_section = pfrom_object->get_output_section(sub_fixup.section()); /// 该fixup所在的output_section
    uint64_t fixup_offset = sub_fixup.offset() + out_offsets[sub_fixup.section()];

    /// 将output_section的Fixup记录到os里，虽然开销很大但不得不做，因为后面的relocate函数处理重定位的时候还需要改动fixup
    output_section->fixup_map[fixup_offset] = add_fixup;

    /// fixup信息迁移
    //add_fixup->set_info(pfrom_object->get_output_section(fixup_input.section())->name());
    add_fixup->set_info(str_concat(pfrom_object->get_output_section(sub_fixup.section())->name(), pfrom_object->name().c_str()));
    add_fixup->set_offset(fixup_offset);
    add_fixup->set_section(output_section->out_shndx());
    add_fixup->set_add(sub_fixup.add());
    if (sub_fixup.has_step())
        add_fixup->set_step(sub_fixup.step());
    /// 这些fixup是从rand节里提取的,在type的第8个bit打上1的标记. 那些后面从重定位中新增的,第8个bit则为默认的0
    add_fixup->set_type( sub_fixup.type() | 128);


    /// 如果base_type和target_type表明存储着符号的话，则重新计算符号的地址
    unsigned base_type = sub_fixup.type() & 3;
    if (base_type == 3 || base_type == 2) // 设置base_type为VALUE
        add_fixup->set_type( (add_fixup->type() & 0xFFFFFFFC) | 0x00000002 );
    // 存储INDEX的情况下，需要将符号转为对应的地址
    if (base_type == 3) {
        uint64_t base_addr = get_symbol_value(pfrom_object, symtab, sub_fixup.base_bbl_sym());
        add_fixup->set_base_bbl_sym(base_addr);
        add_fixup->set_base_section(0);
    }
    // 存储VALUE的情况下，需要将其加上所在节的addr
    else if (base_type == 2) {
        if (sub_fixup.base_section() == 0) {
            gold_info("静态链接时fixup value的base_section为0，说明该子元数据曾经历过链接，请排查该问题 %s 0x%lx", pfrom_object->name().c_str(), sub_fixup.offset());
            exit(0x233);
        }
        add_fixup->set_base_bbl_sym(update_value(pfrom_object, sub_fixup.base_bbl_sym(), sub_fixup.base_section()));
        Output_section* base_output_section = pfrom_object->get_output_section(sub_fixup.base_section()); /// 该fixup的base_section
        add_fixup->set_base_section(0);
    }else{
        add_fixup->set_base_bbl_sym(0);
        add_fixup->set_base_section(0);
    }


    unsigned target_type = (sub_fixup.type() >> 2) & 3;
    if (target_type == 3 || target_type == 2) // 设置target_type为VALUE
        add_fixup->set_type( (add_fixup->type() & 0xFFFFFFF3) | 0x00000008);
    // 存储符号的情况下，需要将符号转为对应的地址
    if (target_type == 3) {
        uint64_t target_addr = get_symbol_value(pfrom_object, symtab,sub_fixup.target_bbl_sym());
        add_fixup->set_target_bbl_sym(target_addr);
        add_fixup->set_target_section(0);
    }
    // 存储offset的情况下，需要将其加上所在节的addr
    else if (target_type == 2){
        if (sub_fixup.target_section() == 0) {
            gold_info("静态链接时fixup value的target_section为0，说明该子元数据曾经历过链接，请排查该问题 %s 0x%lx", pfrom_object->name().c_str(), sub_fixup.offset());
            exit(0x233);
        }
        // 编译器中提供了符号在节中的偏移和节名，需要将其转化为符号在全局中的地址
        add_fixup->set_target_bbl_sym(update_value(pfrom_object, sub_fixup.target_bbl_sym(), sub_fixup.target_section()));
        Output_section* target_output_section = pfrom_object->get_output_section(sub_fixup.target_section()); /// 该fixup的target_section
        add_fixup->set_target_section(0);
    }else{
        gold_info("静态链接时fixup=0x%lx,%s target_type=%d，请排查该问题",
                  fixup_offset, output_section->name(),target_type);
        exit(0x233);
    }
...
template<int size, bool big_endian>
inline uint64_t update_value(Sized_relobj_file<size, big_endian> *pfrom_object, uint64_t offset, int32_t sec_index) {
    /// 符号的全局地址 = (input_section在output_section中的偏移 + output_section的地址) + 符号在input_section中的偏移

    if (pfrom_object->section_exist(sec_index)){
        // 1. input_section在output_section中的偏移
        uint64_t input_section_offset = pfrom_object->get_output_section_offset(sec_index);
        // 2. output_section的地址
        Output_section* output_section = pfrom_object->get_output_section(sec_index);

        return output_section->address() + input_section_offset + offset;
    }else{
        gold_info("静态链接时fixup value type的section为%d，未找到对应的input_section，请排查问题", sec_index);
        exit(0x233);
    }
}

/// xie:
void Layout::add_other_fixup(uint64_t fixup_offset, Output_section* os,
                             uint64_t target_addr, uint64_t base_addr, int64_t add, unsigned int r_type, int fixup_source){

    ShuffleInfo::Metadata_FixupInfo *add_fixup = this->get_global_metadata().add_otherfixups();

    /// 然后设置新的fixup
    add_fixup->set_info(os->name());
    u_int32_t type = 0;
    type |= base_addr==0 ? 0:2;           // base没有的时候传过来0，非0则说明有，注意这里和ps里不一样，因为我们不能简单假设所有reloc的指针都没有base，毕竟got tls还是可能存在的
    type |= 2 << 2;                       // targetType为VALUE
    type |= 0 << 6;                       // 无法判断，因此默认isJump为False
    type |= 0 << 7;                       // 既然是add的other fixup那一定不是RAND来的了
    type |= (fixup_source==1?1:0) << 8;   // isRELOC根据fixup来源设置
    type |= (fixup_source==2?1:0) << 9;   // isGOT根据fixup来源设置

    type |= r_type << 16;                 // 记录重定位类型，r_type是经历了很多层传来的
    add_fixup->set_type(type);

    add_fixup->set_offset(fixup_offset);
    add_fixup->set_section(os->out_shndx());
    add_fixup->set_target_bbl_sym(target_addr);
    add_fixup->set_base_bbl_sym(base_addr);
    add_fixup->set_target_section(0); /// other fixup都是计算后的addr，这个确实没办法转成section+offset的形式，因此这一点是非迭代的
    add_fixup->set_base_section(0);
    add_fixup->set_add(add);
    add_fixup->set_step(1); /// 可能不准确，但大部分otherfixup都是abs，我们也无法得知那些reloc的step说是多少，所以先默认为1吧
}
		 ...
		 Target* target, const Task* task)
{
  ...
  if (!parameters->options().relocatable() && !parameters->doing_static_link())
    {
      ...
      // some information for the dynamic linker. 比如动态符号表。因此doing_static_link模式下 和 保留动态TLS是冲突的。此时允许优化并取消元数据收集
  ...
  /// xie: 步入Segment->OutputSection->OutputData->InputSection来为不同层次的每个元素确定Offset以及Size，其中Size是累加下一层元素得到的。
  /// - set_segment_offsets(). OutputELF级，遍历每个Segment确定其offset和size
  ///   - set_section_addresses(). Segment级，遍历每个Output Section确定offset和size
  ///     - set_section_list_addresses(). Output Section级，遍历每个OutputData确定Offset和Size
  ///       - set_address_and_file_offset(). OutputData级，遍历的好像还是OutputData
  ///         - set_final_data_size(). OutputData级，遍历每个Input Section确定Offset和Size
  do
    {
  ...
  if (ehdr_start != NULL && ehdr_start->is_predefined())
    ...
    /// xie: 判断是否需要元数据合并
    this->is_need_output_rand<64, false>();

    /// xie: 如果本次链接最终的输出中含有元数据，则插入插入rand节。
    if (this->need_output_rand){
        this->rand = this->choose_output_section(NULL, ".rand", elfcpp::SHT_PROGBITS, 0, false, ORDER_INVALID, false, false, false);
        this->rand->set_after_input_sections(); /// 这样该节在Write_after_input_sections_task时才会确定off与size。但注意是在节表后了。
        this->any_postprocessing_sections_ = true; /// 通知gold还需要在结尾的Write_after_input_sections_task再做一遍finalize
    }
  ...
  // sections.
  /// xie: 确定节的off与size，注意节只包括BEFORE_INPUT_SECTIONS_PASS轮次的节，也不包括strtab和shstrtab，这两个节现在还没有create
  ...
  // don't have to wait for the input sections. 确定strtab和shstrtab的off与size
  off = this->set_section_offsets(off, BEFORE_INPUT_SECTIONS_PASS);
  ...
  // Create the section table header. 确定节表的off
  ...
  /// xie: 前面已经finalize了layout，因此非--relocatable下此时可以合并元数据了
  if (this->need_output_rand)
    this->update_shuffleInfo<64, false>(symtab, input_objects);
...
{
    // xie: 加入统计信息的元数据
    ShuffleInfo::Metadata& metadata = this->get_global_metadata();
    ShuffleInfo::Metadata_GoldInfo* gold_info = metadata.mutable_goldinfo();
    gold_info->set_c_fun(this->c_fun);
    gold_info->set_all_obj(this->all_obj);
    gold_info->set_norand_obj(this->hybrid_obj);
    gold_info->set_static_got(this->static_got);
    gold_info->set_silicon_errata(this->silicon_errata);
    gold_info->set_tls_opt(this->tls_opt);
    gold_info->set_got_opt(this->got_opt);
    gold_info->set_got_target(this->got_target);
    gold_info->set_tls_got_target(this->tls_got_target);
    gold_info->set_plt_target(this->plt_target);
    gold_info->set_merged_target(this->merged_target);
    gold_info->set_ifunc_target(this->ifunc_target);
    std::unordered_set<const char*>::iterator missrandfile_iter;
    for (missrandfile_iter=missrandfile.begin(); missrandfile_iter!=missrandfile.end(); missrandfile_iter++){
        gold_info->add_missrandfile(*missrandfile_iter);
    }

    /// xie: 将rand节写进去
    Output_section_data* odata;
    if (this->need_output_rand) {
        /// 将元数据写到rand节中。
        /// 注意，rand节被指定为after_input类型，所以才会在此时finalize。我们需要首先指定size以便于后面的set_section_offset来finalize该节
        /// 该write_section_after_input_sections函数最后会写所有的after_input_sections
        const std::string metadata_str = this->get_global_metadata().SerializeAsString();
        odata = new Output_data_const(metadata_str.c_str(), metadata_str.length(), 1);
        this->rand->add_output_section_data(odata);
