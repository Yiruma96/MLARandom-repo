...
def pointer_state_analysis(elf_path, dataIndex=-1, threadNum=-1):
    ...
    retu = [[C2C, C2D, dict(C_insJMP), dict(C_insJMP_reloc), dict(C_insMEM), dict(C_insMEM_reloc), dict(C_insNOMEM),
             dict(C_insNOMEM_reloc)],
            [D2C, D2D, DIC, D_JPT_num, dict(D_JPT_reloc), D_AbsPtr_num, dict(D_AbsPtr_reloc), D_OtherData_num,
             dict(D_Other_reloc), dict(D_padding)]]
...
if __name__ == '__main__':
    ...
    thread = myThread.myThread(elfList, pointer_state_analysis, poolNum=10,
                               onFinish=pointer_state_finish)  # , onFinish=pointer_state_finish
    thread.start()
