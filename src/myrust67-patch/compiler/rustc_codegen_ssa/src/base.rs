...
pub fn codegen_crate<B: ExtraBackendMethods>(
    ...
    // xie. 在这里开启协调线程之类的，会创建出协调线程处理消息队列，原线程2(是否是主线程？应该不是吧)会继续向下运行来完成codegen
    ...
    // xie. 如果有必要的话，生成一个allocator shim
    // Codegen an allocator shim, if necessary.
    // xie. 如果crate没有设置`allocator_kind`，那么肯定没有shim可以生成。否则，我们也会检查我们所有输出的cockate类型的依赖图。如果有任何东西看起来像 "动态 "链接，那么它已经有了一个分配器垫片，我们将使用它来代替。如果什么都没有，那么我们的工作就是生成分配器!
    ...
    }); // xie. 如果tcx的依赖图中存在任何linkage是Dynamic的话，就要制作这种allocator shim
    let allocator_module = if any_dynamic_crate {
        if tcx.sess.opts.unstable_opts.myalloc{
            println!("[myalloc] None");
        }
        None
        ...
        let module_llvm = tcx.sess.time("write_allocator_module", || {
            backend.codegen_allocator( // xie. allocator shim就是在这里完成的，该函数输出Self.Module
                tcx,
                &llmod_id,
                kind,
        ...
        // xie. 然后用ModuleCodegen包装该Module对象，同时给其赋类型为ModuleKind::Allocator
        // xie. modify2
        if tcx.sess.opts.unstable_opts.myverbose{
            println!("[主线程]   Codegen Alloc CGU: {:?}，通过codegen_allocator函数创建", llmod_id);
        }
        if tcx.sess.opts.unstable_opts.myalloc{
            println!("[myalloc] {:?}", llmod_id);
        }
        Some(ModuleCodegen { name: llmod_id, module_llvm, kind: ModuleKind::Allocator })
    } else {
        // xie. modify2
        if tcx.sess.opts.unstable_opts.myverbose {
            println!("[主线程]   Codegen: 未创建allocator_module");
        }
        if tcx.sess.opts.unstable_opts.myalloc{
            println!("[myalloc] None");
        }
    ...
    // xie. 关于allocator_module，最终在这里，通过submit_pre_codegened_module_to_llvm向协调线程4发送消息
    if let Some(allocator_module) = allocator_module {
        ongoing_codegen.submit_pre_codegened_module_to_llvm(tcx, allocator_module);
    }
    // xie. 接下来才是对hello.rs的处理
    // For better throughput during parallel processing by LLVM, we used to sort
    // CGUs largest to smallest. This would lead to better thread utilization
    // by, for example, preventing a large CGU from being processed last and
    // having only one LLVM thread working while the rest remained idle.
    // xie.  为了在LLVM的并行处理过程中获得更好的吞吐量，我们曾经将CGU从大到小进行排序。这将导致更好的线程利用率，例如，防止一个大的CGU被最后处理，只有一个LLVM线程在工作，而其他线程保持闲置。
    ...
    // xie. 然而，这种策略会导致高内存使用率，因为它意味着所有最大的CGU的LLVM-IR都会同时驻留在内存中。 相反，我们可以通过对CGU进行排序来折中，使最大和最小的CGU排在第一位，第二大和最小的排在下一位，等等。如果有很大的尺寸变化，这可以大大减少内存的使用。
    ...
    // xie. 非并行编译器只能在单线程上将codegen单元转化为LLVM IR，这导致了阶梯效应，即N个LLVM线程必须等待单线程的codegen线程为其生成工作。并行编译器没有这个限制，所以我们可以在把协调工作交给OnGoingCodegen调度器之前，以并行方式预先加载LLVM队列。
    ...
    // xie. 可以看到codegen其实还是串行的，这里会遍历codegen_units中的每一个cgu，通过下面的compile_codegen_unit()函数进行处理
    for (i, cgu) in codegen_units.iter().enumerate() {
        ...
        match cgu_reuse {
            CguReuse::No => {
                ...
                let (module, cost) = if let Some(_cgu) = pre_compiled_cgus.remove(&i) {
                    // xie. modify2
                    if tcx.sess.opts.unstable_opts.myverbose {
                        println!("[主线程]   Codegen: {:?} 从pre_compiled_cgus中提取", cgu.name());
                    }

                    _cgu
                } else { // xie. pre_compiled_cgus中不存在的，主线程2通过backend.compile_codegen_unit()对其进行codegen处理
                    // xie. modify2
                    if tcx.sess.opts.unstable_opts.myverbose {
                        println!("[主线程]   Codegen: {:?} 通过compile_codegen_unit创建", cgu.name());
                    }
                ...
                // xie. 主线程2每处理完一个CGU，通过Message::CodegenDone消息提交给协调线程4
                ...
                false
            }// xie. 如果cgu存在寄存好的结果，直接无需codegen了，可以从bc文件中读取以恢复LLVM IR，然后再交给LLVM工作线程处理为obj file
            CguReuse::PreLto => {
                // xie. modify2
                if tcx.sess.opts.unstable_opts.myverbose {
                    println!("[主线程]   Codegen: {:?} 从preLTO中提取", cgu.name());
                }
            ...
            CguReuse::PostLto => {
                // xie. modify2
                if tcx.sess.opts.unstable_opts.myverbose {
                    println!("[主线程]   Codegen: {:?} 从postLTO中提取", cgu.name());
                }
...
// xie. 判断codegen的cgu是否可以简化其LLVM步骤，包括没有cache时的CguReuse::No，有cache且有LTO时的CguReuse::PreLto，有cache且没有LTO时的CguReuse::PostLto
fn determine_cgu_reuse<'tcx>(tcx: TyCtxt<'tcx>, cgu: &CodegenUnit<'tcx>) -> CguReuse {
    ...
    if tcx.dep_graph.previous_work_product(work_product_id).is_none() {
        ...
        return CguReuse::No; // xie. 之前sess中没有该cgu的cache的话，则为No
    }
    ...
    if tcx.try_mark_green(&dep_node) { // xie. 有cache的话，进一步判断是否做过lto，以选择pre-lto或post-lto
