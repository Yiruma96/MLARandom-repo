...
impl ModuleConfig {
    fn new(
        ...
        ModuleConfig {
            ...
            //     false
            // ),
            emit_asm: sess.opts.output_types.contains_key(&OutputType::Assembly),
...
// xie. 该函数并非进行异步codegen的意思，而是在start_executing_work函数中创建出协调线程4，以方便主线程2继续向下异步的执行codegen
...
fn produce_final_output_artifacts(
    ...
    let copy_if_one_unit = |output_type: OutputType, keep_numbered: bool| {
        if compiled_modules.modules.len() == 1 { // xie. 当compiled_modules里只有1的时候，只要改个名字即可  (copy `foo.0.x` to `foo.x`)
            // 1) Only one codegen unit.  In this case it's no difficulty
            //    to copy `foo.0.x` to `foo.x`.
            let module_name = Some(&compiled_modules.modules[0].name[..]);
            let path = crate_output.temp_path(output_type, module_name);
            // xie. modify2
            if sess.opts.unstable_opts.myverbose{
                println!("[主线程]   拷贝{:?}到{:?}", &path, &crate_output.path(output_type));
            }
        ...
        } else { // xie. 当compiled_modules里有多个的时候，我们需要将他们合并到一起
    ...
    for output_type in crate_output.outputs.keys() {
        match *output_type {
            ...
            OutputType::Assembly => { // xie. 确实，--emit=obj,asm都是缺少alloc的，原来他们两个在一起
                copy_if_one_unit(OutputType::Assembly, false);

                // xie. modify3 单独拷贝allocate module
                if sess.opts.unstable_opts.myoutput {
                    if let Some(allocator_module) = &compiled_modules.allocator_module {
                        let mut alloc_path = crate_output.path(OutputType::Assembly).clone();
                        match alloc_path.file_name() {
                            Some(file_name) => {
                                let temp_path = format!("{}{}", file_name.to_str().unwrap(), ".alloc.s");
                                alloc_path.set_file_name(temp_path);

                                if sess.opts.unstable_opts.myverbose {
                                    println!("[主线程]   拷贝allocator_module从{:?}到{:?}",
                                             crate_output.temp_path(OutputType::Assembly, Some(&allocator_module.name)),
                                             alloc_path);
                                }

                                copy_gracefully(&crate_output.temp_path(OutputType::Assembly, Some(&allocator_module.name)),
                                                &alloc_path);
                            },
                            None => println!("[主线程]   拷贝allocator_module时未发现合适的file_name,请排查原因")
                        }
                    }
                }
            }
            ...
            OutputType::Mir | OutputType::Metadata | OutputType::Exe | OutputType::DepInfo => {} // xie. Exe的意思是，已经生成完成了？？？
        }
    }
    ...
    // xie. 开始清理临时文件，可以看到没有save_temps参数的时候才会执行这里
...
fn execute_work_item<B: ExtraBackendMethods>(
    ...
    // println!("[LLVM工作线程] 开始处理Module {}", module)
    match work_item {
        WorkItem::Optimize(module) => {
            // xie. modify2
            if cgcx.opts.unstable_opts.myverbose {
                println!("[LLVM工作线程 Thread-{:?}] 开始处理普通Module {:?}", thread::current().id(), module.name);
            }
        ...
        WorkItem::CopyPostLtoArtifacts(module) => {
            // xie. modify2
            if cgcx.opts.unstable_opts.myverbose {
                println!("[LLVM工作线程 Thread-{:?}] 开始处理普通PostLto Module {:?}", thread::current().id(), module.name);
            }
            Ok(execute_copy_from_cache_work_item(cgcx, module, module_config))
        }
        WorkItem::LTO(module) => {
            // println!("[LLVM工作线程] 开始处理普通LTO Module {:?}", module.name);
...
fn execute_optimize_work_item<B: ExtraBackendMethods>(
    ...
    // xie. 后面的工作过程中会多次见到B::optimize()，为什么？？？
    unsafe {
    ...
    // xie. 在执行代码降级或者执行LTO优化之前，先存储bc文件到磁盘上
    ...
    match lto_type {
        ComputedLtoType::No => finish_intra_module_work(cgcx, module, module_config), // xie. LLVM工作线程会在这里处理普通的非LTO优化的Module
...
fn start_executing_work<B: ExtraBackendMethods>(
    ...
    let helper = jobserver
        .into_helper_thread(move |token| {  // xie. 这里会再创建一个线程3，看注释的意思是将jobserver转为一个独立的helper thread，
            drop(coordinator_send2.send(Box::new(Message::Token::<B>(token)))); // xie. 调试来看，这个Token会在每次LLVM工作线程发出Done之后2稳定触发
        })
    ...
    // xie. modify2
    let myverbose_for_thread4: bool = sess.opts.unstable_opts.myverbose;
    if sess.opts.unstable_opts.myverbose {
        println!("[主线程] 创建出协调线程");
    }

    return B::spawn_thread(cgcx.time_trace, move || { // xie. spawn_thread()调用thread::spawn()，又会将参数2的闭包函数放到新线程里运行，这里面的内容就是协调线程4了
        ...
        let mut compiled_modules = vec![]; // xie. 收集完成codegen和LLVM两个步骤的obj file
        ...
        let mut codegen_done = false; // xie. codegen指Source Code -> LLVM IR，由于codegen通常比LLVM快，所以该标志位有可能用于将主线程的token释放？

        // This is the queue of LLVM work items that still need processing.
        let mut work_items = Vec::<(WorkItem<B>, u64)>::new(); // xie. 协调线程负责维护LLVM待执行队列，里面是已经完成Source Code -> LLVM IR的，该队列是compiled_modules的半成品
        ...
        let mut tokens = Vec::new(); // xie. 协调线程负责下发token

        let mut main_thread_worker_state = MainThreadWorkerState::Idle; // xie. 主线程有三种状态 在codegen_done之前为Idle和Codegening两种，在codegen_done之后为LLVMing，也就是变身为LLVM工作线程
        let mut running = 0;
        ...
        // work to be done.
        let mut message_round: i128 = 0; // xie. modify2
        while !codegen_done
            || running > 0
                    ...
                    && main_thread_worker_state == MainThreadWorkerState::Idle))
        {
            ...
            if !codegen_done {
                if main_thread_worker_state == MainThreadWorkerState::Idle {
                    ...
                    if !queue_full_enough(work_items.len(), anticipated_running) {
                        ...
                        if codegen_worker_send.send(Message::CodegenItem).is_err() { // xie. 向主线程发送一个Codegen结果包装任务，同时修改主线程工作状态
                            panic!("Could not send Message::CodegenItem to main thread")
                        }
                        main_thread_worker_state = MainThreadWorkerState::Codegenning;
                    } else { // xie. 另一种情况是，workitem的长度已经足够让LLVM工作线程都不停歇了，所以让主线程干LLVM工作线程的事情
                        ...
                        main_thread_worker_state = MainThreadWorkerState::LLVMing;
                        // xie. modify2
                        if myverbose_for_thread4 {
                            println!("[协调线程] [Round#{:?}] Send LLVM {:?} (workitem过长因此该LLVM由主线程援助) ", message_round, item.short_description());
                        }
                ...
                // xie. 正如我们上面所推测的，当codegen_done结束时候，主线程会变为LLVMing状态，来完成和LLVM工作线程一样的事情
                ...
                match main_thread_worker_state {
                    MainThreadWorkerState::Idle => {
                        if let Some((item, _)) = work_items.pop() {
                            ...
                            // xie. modify2
                            if myverbose_for_thread4 {
                                println!("[协调线程] [Round#{:?}] Send LLVM {:?} (主线程结束因此该LLVM由主线程援助)", message_round, item.short_description());
                            }
                            spawn_work(cgcx, item);
                        } else {
            ...
            // xie. 每一轮检查时，如果还有剩余的token(running < tokens.len())，协调线程会从work_item队列中取出(item=work_items.pop().unwrap();)，并开始一个LLVM工作线程(spawn_work(cgcx, item);)
            ...
            while !codegen_aborted && !work_items.is_empty() && running < tokens.len() {
                ...
                // xie. modify2
                if myverbose_for_thread4 {
                    println!("[协调线程] [Round#{:?}] Send LLVM {:?}", message_round, item.short_description());
                }
                spawn_work(cgcx, item);
                running += 1;
            }
            ...
            // xie. 协调者线程接收消息并处理，然后循环重新开始上述的决策行为
            let msg = coordinator_receive.recv().unwrap();
            match *msg.downcast::<Message<B>>().ok().unwrap() {
                // Save the token locally and the next turn of the loop will use
                // this to spawn a new unit of work, or it may get dropped
                // immediately if we have no more work to spawn.
                Message::Token(token) => { // xie. jobserver所在的help thread，也就是线程3所发来的消息，起到回收token的作用，其用意目前仍然不清楚
                    match token {
                        Ok(token) => {
                            tokens.push(token);
                ...
                // xie. 线程2发来的消息，用于告知协调线程一个Codegen完成了。协调线程会将其放到work_item中
                Message::CodegenDone { llvm_work_item, cost } => {
                    // xie. modify2
                    if myverbose_for_thread4 {
                        println!("[协调线程] [Round#{:?}] Receive Codegen Done {:?}", message_round, llvm_work_item.short_description());
                    }
                ...
                // xie. 主线程2在创建出协调线程后，会继续进行codegen生成，codegen执行完毕时线程2会发出该信号给协调线程。注意codegen_done标志会使主线程2进入LLVMing状态
                Message::CodegenComplete => {
                    ...
                    // xie. modify2
                    if myverbose_for_thread4 {
                        println!("[协调线程] [Round#{:?}] Receive All Codegen Done", message_round);
                    }
                }
                ...
                } // xie. LLVM工作线程传来的结束信息，根据Module类型不同，收集到compiled_modules或compiled_allocator_module中。注意compiled_allocator_module只能有一个
                Message::Done { result: Ok(compiled_module), worker_id } => {
                    ...
                    match compiled_module.kind {
                        ModuleKind::Regular => {
                            // xie. modify2
                            if myverbose_for_thread4 {
                                println!("[协调线程] [Round#{:?}] Receive LLVM Done {:?} Regular", message_round, compiled_module.name);
                            }
                        ...
                        ModuleKind::Allocator => {
                            // xie. modify2
                            if myverbose_for_thread4 {
                                println!("[协调线程] [Round#{:?}] Receive LLVM Done {:?} Allocator", message_round, compiled_module.name);
                            }
        ...
        Ok(CompiledModules { // xie. 进行编译，将相关的obj files加入CompiledModules，后面link会从sess中取出来CompiledModules来构建link_cmd
            ...
            fn drop(&mut self) {
                ...
                let msg = match self.result.take() {
                    Some(Ok(WorkItemResult::Compiled(m))) => {
                        Message::Done::<B> { result: Ok(m), worker_id } // xie. 这里看起来是LLVM工作线程的代码，其在运行结束后会发出Done信息通知协调线程4
                    }
...
impl<B: ExtraBackendMethods> OngoingCodegen<B> {
    pub fn join(self, sess: &Session) -> (CodegenResults, FxHashMap<WorkProductId, WorkProduct>) {
        ...
        self.shared_emitter_main.check(sess, true); // xie. 可以看到，虽然link接收了queries.onGoingCodegen，并由onGoingCodegen.join()得到CodegenResult，但该值还是来自sess
        ...
        (
            CodegenResults {
                ...
                modules: compiled_modules.modules, // xie. 在这里，将OngoingCodegen的modules传递到CodegenResults中
