...
fn link_natively<'a>(
    ...
    codegen_results: &CodegenResults,
    tmpdir: &Path,
    outputs: &OutputFilenames // xie. modify
) -> Result<(), ErrorGuaranteed> {
...
fn add_linked_symbol_object(
    ...
    // xie. modify3 保存该symbol.o
    if sess.opts.unstable_opts.myoutput {
        let mut symbol_path = outputs.path(OutputType::Object).clone();
        match symbol_path.file_name() {
            Some(file_name) => {
                let temp_path = format!("{}{}", file_name.to_str().unwrap(), ".mysymbol.o");
                symbol_path.set_file_name(temp_path);
            },
            None => println!("未发现合适的file_name,请排查原因")
        }

        // xie. modify2
        if sess.opts.unstable_opts.myverbose{
            println!("[主线程]   发出了{:?}，我们将其拷贝到其他目录下{:?}", &path, symbol_path);
        }

        if let Err(e) = fs::copy(&path, &symbol_path) {
            sess.emit_err(errors::CopyPath::new(&path, &symbol_path, e));
        }
    }
