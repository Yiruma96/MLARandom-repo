...
pub fn target_machine_factory(
    ...
    // xie. modify 用于后面的obj分步骤执行
    let fno_integrated_as = sess.opts.unstable_opts.my_fno_integrated_as;
    ...
    Arc::new(move |config: TargetMachineFactoryConfig| {
        ...
        // xie. modify2 Rust最终在这里调用创建TargetMachine的构造函数。值得对比的是，原LLVM是在EmitAssembly()这一代码生成的入口，才通过CreateTargetMachine函数构造TargetMachine的
        ...
        let tm = unsafe {
            ...
            llvm::LLVMRustCreateTargetMachine(
                ...
                fno_integrated_as, // xie. modify 将命令行中的参数传到TargetMachine.options中去
            )
        };
    ...
    let _timer = cgcx.prof.generic_activity_with_arg("LLVM_module_codegen", &*module.name);
    {
        ...
        if config.emit_ir {
            ...
            if cgcx.opts.unstable_opts.myverbose {
                println!("[LLVM工作线程 Thread-{:?}] 为Module {:?}发出LLVM IR File", thread::current().id(), module.name);
            }
        ...
        // xie. modify2
        if cgcx.opts.unstable_opts.myverbose {
            println!("[LLVM工作线程 Thread-{:?}] Module {:?}的config.emit_asm为{:?}", thread::current().id(), module.name, config.emit_asm);
        }
        if config.emit_asm {
            let _timer =
            ...
            // xie. modify2
            if cgcx.opts.unstable_opts.myverbose {
                println!("[LLVM工作线程 Thread-{:?}] 为Module {:?}发出ASM File", thread::current().id(), module.name);
            }
        ...
        match config.emit_obj {
            EmitObj::ObjectCode(_) => {
                ...
                if cgcx.opts.unstable_opts.my_fno_integrated_as && (cgcx.target_arch=="x86_64" || cgcx.target_arch=="aarch64") {
                    // xie. modify 我们首先调用LLVM库来生成as，然后再主动调用gas来汇编该文件
                    if cgcx.opts.unstable_opts.myverbose {
                        println!("[LLVM工作线程 Thread-{:?}] 为Module {:?}通过中间汇编发出Obj File", thread::current().id(), module.name);
                    }
                    // xie. Step 1. LLVM生成as参考clang -cc1 -no-integrated-as命令的行为, 在创建TargetMachine时给TargetOptions设置DisableIntegratedAS
                    let temp_as_out = cgcx.output_filenames.temp_path(OutputType::Assembly, module_name);
                    ...
                    // xie. Step 2. 主动调用gas, 参考clang包装器所创建的Assembler::ConstructJob()的实现
                    // xie. 为了便于调用LLVM lib的接口，我们将该函数实现在C语言中，对应的这个Rust函数只要在ffi.rs中声明就可以了。
                    // xie. 值得一提的是，我们一开始准备调用clang::driver::main()来执行命令(给argv适当传入即可)，但后面发现Clang的Driver是没有链接的，只有LLVM的库函数链接到了Rust中
                    // xie. 所以我们现在还是准备通过命令执行来直接执行gas --64 -o main2.o main2.s
                    let c_string = CString::new(&*cgcx.target_arch).unwrap();
                    let retu_code = llvm::LLVMExecGAS(c_string.as_ptr(), path_to_c_string(&temp_as_out).as_ptr(), path_to_c_string(&obj_out).as_ptr());

                    // xie. 如果执行成功则删除中间汇编文件，执行失败的话可以保留该文件以待后续分析
                    if retu_code == 0 {
                        fs::remove_file(&temp_as_out).expect("删除文件失败");
                    }
                    // xie. 如果执行失败，大概率是由于rust发出的汇编代码无法被gas处理所导致，此时我们回退用原生的发出策略
                    else{
                        println!("[LLVM工作线程 Thread-{:?}] 为Module {:?}构建的汇编文件{:?}无法通过编译，请排查原因", thread::current().id(), module.name, &temp_as_out);
                        with_codegen(tm, llmod, config.no_builtins, |cpm| {
                            write_output_file(
                                diag_handler,
                                tm,
                    ...
                    if cgcx.opts.unstable_opts.myverbose {
                        println!("[LLVM工作线程 Thread-{:?}] 为Module {:?}直接发出Obj File", thread::current().id(), module.name);
                    }
                    // xie. modify 原代码是直接调用的生成接口
                    with_codegen(tm, llmod, config.no_builtins, |cpm| {
                        write_output_file(
                            diag_handler,
                            tm,
