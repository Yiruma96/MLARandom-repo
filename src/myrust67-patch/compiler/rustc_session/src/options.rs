...
options! {
    ...
    // xie. modify 增加一些控制参数
    myverbose: bool = (false, parse_bool, [UNTRACKED],
        "打印Rust编译过程(default: no)"),
    myoutput: bool = (false, parse_bool, [UNTRACKED],
        "用于Rust编译过程拆分后的补充输出：在emit=link时额外输出symbols.o，在emit=asm时额外输出alloc.o(default: no)"),
    myalloc: bool = (false, parse_bool, [UNTRACKED],
        "打印alloc object的名字，以便于我们修改链接命令时替换掉该文件 (default: no)"),
    my_fno_integrated_as: bool = (true, parse_bool, [UNTRACKED],
        "类似于clang的-fno-integrated-as参数，我们在write.rs:codegen():match config.emit_obj{}中，将obj编译命令拆分为两步 (default: no)"),
