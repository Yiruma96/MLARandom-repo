...
pub fn run_compiler<R: Send>(config: Config, f: impl FnOnce(&Compiler) -> R + Send) -> R {
    ...
    // xie. modify2
    if config.opts.unstable_opts.myverbose {
        println!("[初始线程] 创建出主线程");
    }

    util::run_in_thread_pool_with_globals( // xie. 该函数会创建线程2来完成具体的编译过程，注意编译过程并非run_in_thread_pool_with_globals实现，而是位于参数三的闭包函数中
        config.opts.edition,
        ...
        || {
            ...
            rustc_span::with_source_map(compiler.sess.parse_sess.clone_source_map(), move || {
                let r = {
                    ...
                    f(&compiler) // xie. 在线程2中终于一层层的调用到了这个run_compiler中的闭包函数
                };
