...
pub(crate) fn run_in_thread_pool_with_globals<F: FnOnce() -> R + Send, R: Send>(
    ...
    // xie. The "thread pool" is a single spawned thread in the non-parallel 这里就是个单线程来完成编译的，作者解释为保护栈大小，以及尽可能的贴近并行编译器的实现方式
    ...
    thread::scope(|s| { // xie. thread::scope(fn)是1.63后引入的新特性，总体上和thread::spawn(fn)相同，都是开启一个新的线程来运行参数fn. 该函数会创建线程2
        // `unwrap` is ok here because `spawn_scoped` only panics if the thread
        // name contains null bytes.
        let r = builder
            .spawn_scoped(s, move || rustc_span::create_session_globals_then(edition, f))// xie. 子线程运行第二个参数闭包函数开始，也就是create_session_globals_then函数会运行在线程2中，起到具体编译的是闭包函数f
            .unwrap()
            .join();             // xie. 父线程通过.join()阻塞一直到子线程rustc2运行结束
