...
// xie. modify 用于命令执行的fork,wait,execvp函数需要该头文件
#include <sys/types.h>
#include <sys/wait.h>
#include <unistd.h>
#include <string.h>
...
// xie. modify 该函数在ffi.rs中声明了，所以可以直接被write.rs调用。我们在这里给出该函数的具体实现。
// xie. 一开始我们准备参考clang::driver::main()来调用接口实现clang -fno-integrated-as -c -o output.o input.s, 但发现clang的include并没有导出
//      所以我们还是通过命令执行来做。
extern "C" int LLVMExecGAS(const char* arch, const char* input_path, const char* output_path){
    //  xie. 直接通过命令执行实现

    // xie. 拼接汇编命令
    int input_path_length = strlen(input_path);
    char input_path_temp[input_path_length+1];
    strcpy(input_path_temp, input_path); // input_path来自Rust内存对象，为了避免破坏它们，我们使用strcpy建立一个拷贝
    int output_path_length = strlen(output_path);
    char output_path_temp[output_path_length+1];
    strcpy(output_path_temp, output_path);

    // xie. 使用exec，execvp执行命令，注意一定要fork到子进程中执行，因为exec类会替换当前进程，只有在执行失败时才会返回-1
//    char* gas_cmd[6];
//    gas_cmd[0] = "as-print";
...
//    }
//    gas_cmd[2] = "-o";
//    gas_cmd[3] = output_path_temp;
//    gas_cmd[4] = input_path_temp;
//    gas_cmd[5] = (char *) NULL;
//
//    if(fork()){
//        int status = 0;
//        wait(&status);
//
//        // xie. 就因为这个分析了半天，确认是阻塞函数wait结束后，有个窗口期子进程的命令还在执行，所以我们得引入一个小延迟。相反的，popen不需要fork，就没有这个问题
//        sleep(0.5);
//        std::remove(input_path_temp);
//
//        if (WEXITSTATUS(status) == 1) {
//            printf("Failed to %s Assembly %s to %s\n", arch, input_path_temp, output_path_temp);
//            return 1;
//        }
//    }else{
//        int subcmd_status = execvp(gas_cmd[0], gas_cmd);
//        if (subcmd_status == -1)
//            exit(1);
//    }

    // xie. 使用popen或system来执行汇编命令, 该函数不会替换当前进程，无需fork也就不存在前面两个进程间奇怪的同步问题
    std::string gas_cmd = "as ";
    if (strcmp(arch, "aarch64") == 0) {
        gas_cmd.append("-EL -o");
        gas_cmd.append(output_path_temp);
        gas_cmd.append(" ");
        gas_cmd.append(input_path_temp);
    }else if (strcmp(arch, "x86_64") == 0) {
        gas_cmd.append(" --64 -o ");
        gas_cmd.append(output_path_temp);
        gas_cmd.append(" ");
...
//    xie. 参考clang::driver::main()的接口实现
//    int argc_ = 6;
//    const char* argv_[] = {"clang", "-fno-integrated-as", "-c", output_path, "-o", input_path};
...
//
//        if (Res != 0){
//            printf("Failed to exec: clang -fno-integrated-as -c %s -o %s\n", output_path, input_path);
//        }
//    }
//    return Res;
...
extern "C" LLVMTargetMachineRef LLVMRustCreateTargetMachine(
    ...
    bool fnoIntegratedAS // xie. modify 相应的修改参数
    ) {
  ...
  // xie. modify Rust执行过程会在这里生成TargetMachine，从LLVM调试可知TargetOptions里面就有fno-integrated-as的控制！！！，我们在这里做参数中转即可
  // xie. 我们首先确认了是否开启fno-integrated-as，其中端的LLVM IR是一致的，导致汇编不一致的原因就是在于TargetOptions的不一致
  // xie. 通过对比clang -S -o main2.s main.c -###和clang -S -o main2.s main2.c -fno-integrated-as -###，我们确认了以下三项参数存在不同
  // xie. 1. 新增-no-integrated-as: 写入到TargetOptions.DisableIntegratedAS
  // xie. 2. 新增-fno-dwarf-directory-asm:  写入到TargetOptions.MCOptions.MCUseDwarfDirectory
  // xie. 3. 删除-faddrsig: 写入到TargetOptions.EmitAddrsig
  /* class TargetOptions {
   *    unsigned DisableIntegratedAS : 1;
   * }
   *
   * targetMachine = createTargetMachine(..., targetOptions, ...)
   *
  ...
  if (fnoIntegratedAS) {
      ...
      Options.EmitAddrsig = 0;
  }else{
      Options.DisableIntegratedAS = 0;
      Options.MCOptions.MCUseDwarfDirectory = llvm::MCTargetOptions::EnableDwarfDirectory;
      Options.EmitAddrsig = 1;
  }
