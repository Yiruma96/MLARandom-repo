...
fn run_compiler(
    ...
    // xie. interface::run_compiler就是编译的开始，它包括来自命令行参数的config，以及一个接受Compiler参数的闭包函数。注意闭包函数会运行在子线程2中
    interface::run_compiler(config, |compiler| {
        ...
        // xie. 在子线程中，compiler.enter()完成具体的编译过程,经历start_codegen, start_executing_work等，最终完成编译。
        let linker = compiler.enter(|queries| {
            ...
            // xie. 这里完成编译的主体工作，该函数会创建出协调线程，之后进行codegen，而协调线程会创建出LLVM工作线程
