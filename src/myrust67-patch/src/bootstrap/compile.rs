...
impl Step for Std {
    ...
    fn run(self, builder: &Builder<'_>) {
        ...
        // xie
        println!("[*] Std Cargo!!!");
        let _cur_envs: Vec<(&OsStr, Option<&OsStr>)> = cargo.get_env();
        for env in _cur_envs {
            if let Some(env1) = env.1 {
                println!("  export {}=\"{}\"", env.0.to_str().unwrap(), env1.to_str().unwrap());
            }
        }
