...
static unsigned int dyn_line;  /// xie: 追加的动态行号
...
input_scrub_new_file (const char *filename)
{
  ...
  dyn_line = 0; /// xie: read_source函数的头部会调用该函数，我们也要将dyn_line置为0
...
/// xie: 根据read_source的读取次数累加动态行
void
dyn_line_counters (void)
{
    dyn_line++;
}
...
/// xie:
unsigned int cur_dyn_line (void)
{
    return dyn_line;
}
