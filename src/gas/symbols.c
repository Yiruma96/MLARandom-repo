       ...
       /* We copy this string: OK to alter later.  */)
{
  ...
  /// xie: 在sy_hash两个符号表中查找是否有冲突的符号, symbol_find大致就是这样实现的
  if ((symbolP = symbol_find (sym_name)) != 0)
    {
      /// 进入这里之前已经有指令创建过该符号了，比如.global f1创建符号f1，然后.type f1,%function给该符号赋为function的flags
...
resolve_symbol_value (symbolS *symp)
{
  ...
  segT final_seg;
    ...
    /// xie:
    /// 对于local_symbol来说，计算方式很容易
    /// symbol中的value里存放着在frag中的偏移。在frag确定后，frag->fr_address + symbol.lsy_value = symbol_value
    /// 对于全局符号来说，计算好的值存放在value.add_number中
    /// 其计算方式比较困难，要因为symbol.value是一个expression，要根据expression的op来计算
...
{
    /// xie: 本地符号和未解决的全局符号，都使用resolve_symbol_value函数先resolve
  ...
  if (!s->flags.resolved)
    ...
    /// xie: 如果是弱符号的话，值存储在value.add_symbol里
    /// 其他情况下，应该都存放在value.add_number中
  if (S_IS_WEAKREFR (s))
