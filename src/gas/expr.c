      ...
      enum expr_mode mode	/* Controls behavior.  */)
{
  ...
  /// xie: 解析左操作数，返回到resultP里面，如果左操作数是表达式则会进行递归   返回的retval是符号所在的节
  ...
  while (op_left != O_illegal && op_rank[(int) op_left] > rank)
    {
      ...
      right.X_md = 0;
      /// xie: 解析右操作数，返回到right里面，同样如果右操作数是表达式则会进行递归，返回的rightseg是符号所在节
...
# define md_register_arithmetic 1
      ...
      /// xie: 根据操作码op_left和操作数resultP及right，来确定是计算出还是存储到fix里面
	  ...
	  && (md_register_arithmetic || resultP->X_op != O_register))
	{
      ...
      /// xie: 其实这种提前的计算是专门对pic的，也就是左右操作数都是符号并且操作码为减法时
      /// 这时候，如果左右操作数的节一样(resultP是左操作数的节，retval是右操作数的节)，并且满足S_FORCE_RELOC和symbol_get_frag相等一系列关系时候
      /// 则提前计算出两个符号间的差值
      /// 将其注释掉，以使这类fixup写入到fixup里面
