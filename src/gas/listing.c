...
list_info_type* listing_newline (char *ps)
{
  const char *file;
  unsigned int line;
  static unsigned int last_dyn_line = 0xffff; /// xie: static全局变量用于，用于记录上一次步入该函数时的dyn_line
  ...
  /// 在源文件中定位该行，其实就是从一个全局的logical_input_line或physical_input_file变量中取出来
  if (listing & LISTING_HLL)
    file = as_where (&line);
  else
    file = as_where_physical (&line);

  unsigned int dyn_line = cur_dyn_line(); /// xie:

  if (ps == NULL)
    {
      if (dyn_line == last_dyn_line
    ...
    /// list_info_type->line_contents为NULL
  ...
  last_dyn_line = dyn_line; /// xie:
  last_line = line;
  last_file = file;

  /*
   * first_new_frag -> listing_tail
   * new_listing -> first_new_frag
   * second_new_frag -> new_listing
   *
   * 可以看到两个特别的s点：
   * - 每个listing有两个frag，并且second_frag是下一行的时候才创建并绑定的
   * - listing->frag并非自己的第一个frag，而是上一个listing的second_frag，所以最后print_list的时候要有个while来向后找到真正的两个frag
   * */
  /// 创建新的frag_now，该frag_now实际上是作为上一个list_info_type的second_frag，因为要注意此时listing_tail还没有更新
  new_frag ();
  ...
  /// list_info_type->frag实际指向的是上一个list_info_type的second_frag。所以最后print_list的时候要有个while来向后找到真正的frag
  /// 个人猜测一行对应两个frag是因为mips架构可能扩充NOP指令，而这些指令应该是放在second_frag中的。(见listing_prev_line函数)
  /// 但为何list_info_type要指向上一个上一个list_info_type的second_frag，属实是没搞懂，和打印没有关系因为要while掉，可能还是和mips有关吧。
  new_i->frag = frag_now;
  new_i->line = line;      /// 行号从前面提出来
  new_i->dyn_line = dyn_line; /// xie:
  new_i->file = file_info (file); /// 每次都从调用file_info()，这一步比较耗时而且没必要
  ...
  /// 又创建了一个新的frag，由于此时listing_tail已经更新，这个frag才是list_info_type的first_frag
  new_frag ();
...
calc_hex (list_info_type *list)
{
  ...
  /// 这里假设了一行拥有多个frag，所以默认创建的第二个frag也是它的
  /* Dump all the frags which belong to this line.  */
  while (frag_ptr != (fragS *) NULL && frag_ptr->line == first)
    {
      /* Print as many bytes from the fixed part as is sensible.  */
      octet_in_frag = 0;
      /// 指令byte包括两部分，第一部分是常见的frag_ptr->fr_fix
      while (octet_in_frag < frag_ptr->fr_fix
	     && data_buffer_size < MAX_BYTES - 3)
	{
      /// 指令地址是第一个frag的address
      ...
      if (frag_ptr->fr_type == rs_fill)
	{
      ...
      /// 除了fr_fix部分外，还会写fr_var*fr_offset部分，目前没搞懂这部分是什么，但应该是确实存在的。所以说还是得用address计算指令长度...
	  while ((octet_in_frag
...
listing_listing (char *name ATTRIBUTE_UNUSED)
{
  ...
  /// list_info_type*单链表数组list，以源文件的行为元素单位，记录了该行对应的十六进制情况
  while (list)
    {
...
//	      while (list->file->linenum < list_line
//		     && !list->file->at_end)
          /// xie: 调整下，不然一个line里有多个指令虽然会被分配到不同list_info_type里，但会因为linenum<list_line而不打印
          bfd_boolean has_print = FALSE;
          while (!list->file->at_end)
          {
              if (list->file->linenum < list_line) {
                  /// 正常的读取，注意该分支可能是多次被遍历到的，因为源码行可能是跨行或者多个buffer的
                  ...
                  if (list->file->linenum < list_line)
                      ...
                      /// 根据list_info_type得到该行在frag中对应的十六进制，并写到data_buffer中
                      address = calc_hex(list);

                  /// 打印p中记录的源文件行，以及data_buffer中记录的十六进制
                  if (!((listing & LISTING_NODEBUG) && debugging_pseudo(list, p))) {
                      print_lines(list, list->file->linenum, p, address);
                      has_print = TRUE;
                  }
              }else{
                  /// xie: 因为我们现在以动态行为list_info_type的单位，所以会有list_info_type的行和上一个一样的情况，
                  /// 这时候由于buffer_line在前面已经读取完毕了，所以会运行出错，所以我们开启这个分支处理这种情况
                  if (!has_print) {
                      unsigned int address;
                      address = calc_hex(list);
                      if (address != ~(unsigned int)0)
