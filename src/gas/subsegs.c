...
subseg_set_rest (segT seg, subsegT subseg)
{
  ...
  frag_now = frcP->frch_frag_now;
    ...
    /// xie: .section .text 导致的环境切换在这里发生。 我需要将当前的now_sec_layout进行切换
    char* sec_id1 = int2str(seg->id);
    if ((now_instList = str_hash_find(sec2inst_map, sec_id1)) == NULL){
        now_instList = malloc(sizeof(inst_list));
        memset(now_instList, 0, sizeof(label_list));
        str_hash_insert(sec2inst_map, sec_id1, now_instList, 1);
    }else{
        free(sec_id1); /// 之前的修改中曾将sec_id1都free，这导致hash_map中的value无法再被寻址到，所以这个key是要分配在堆上全局保存的
    }
    char* sec_id2 = int2str(seg->id);
    if ((now_labelList = str_hash_find(sec2label_map, sec_id2)) == NULL){
        now_labelList = malloc(sizeof(label_list));
        memset(now_labelList, 0, sizeof(label_list));
        str_hash_insert(sec2label_map, sec_id2, now_labelList, 1);
    }else{
        free(sec_id2);
    }
