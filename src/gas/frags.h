...
struct frag {
  ...
  addressT fr_address;             /// 该frag存放的bytes在section中的偏移
  /* When relaxing multiple times, remember the address the frag had
     in the last relax pass.  */
  addressT last_fr_address;

  /* (Fixed) number of octets we know we have.  May be 0.  */
  valueT fr_fix;                   /// 大概是该frag里存放了多少byte，但目前不清楚是否包括padding，所以计算指令长度最好还是根据下一个frag的address之差算
  /* May be used for (Variable) number of octets after above.
     The generic frag handling code no longer makes any use of fr_var.  */
  offsetT fr_var;
  /* For variable-length tail.  */ /// 加数
  offsetT fr_offset;
  /* For variable-length tail.  */ /// Target
  symbolS *fr_symbol;
  ...
  /* Data begins here.  */ /// frag存储数据的位置。看起来是放在结构体内部而非用指针指向，那这种结构体的长度不确定，不会容易溢出吗？？？
  char fr_literal[1];
};
