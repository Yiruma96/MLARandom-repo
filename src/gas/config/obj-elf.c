			...
			int push)
{
  ...
  if (old_sec)
    ...
    sec = subseg_force_new (name, 0); /// xie: 这里创建出这个节
  ...
  if (old_sec == NULL)
	...
	symbol_table_insert (section_symbol (sec)); /// xie: 创建该节的符号
    }
  else
    {
      if (type != SHT_NULL
...
obj_elf_type (int ignore ATTRIBUTE_UNUSED)
{
  ...
  /// 根据sym_name首先定位出SymbolS sym，然后再定位出asymbol(elfsym->symbol)，该对象里面存放符号的属性，可以识别是否为函数的开头
  sym = get_sym_from_input_line_and_check ();
  c = *input_line_pointer;
  ...
  *input_line_pointer = c;
  ...
  if (!md_elf_symbol_type_change (sym, elfsym, type))
      ...
      if (type != BSF_OBJECT)
	{
      ...
      if (type)
	{
      /// 或的方式将新type和原来的flags合并，然后放到符号的flags里面
