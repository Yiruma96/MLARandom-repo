...
#else
/// xie. 原本为"/"，我们将其修改为"#/"。i386_comment_chars被用作行内注释符，下面的line_comment_chars被用作单独的注释行。
const char *i386_comment_chars = "#/";
#define PREFIX_SEPARATOR '/'
#endif
...
/// xie: 调用自read.c的read_a_source_file。 除了标签符号和handler之外，汇编指令是machine-dependent的，因此要在对应的架构代码中处理
void
...
/// xie: parse_insn(line)    解析指令！！！  解析结果分为三部分，操作码放在mnemonic中，操作数放在line中。 i.suffix中存放后缀如pushq的q
  line = parse_insn (line, mnemonic);
  if (line == NULL)
    return;
  mnem_suffix = i.suffix;
/// xie: parse_operands(line)   解析操作数！！！，结果放到放到i386_insn i中，比如i.opearands记录操作数个数，reg_operands记录寄存器操作数个数
...
/// xie: 根据存在mnem_suffix中的指令后缀以及操作数，来选择合适的模板：即insn.tm
  ...
  /// xie: 对于有操作数的指令，其操作码还要进一步计算，即不能直接用insn_template里的base_opcode
  if (i.operands) /// 该函数会检查指令模板中i.tm.opcode_modifier,这里面记录的是指令后的修饰符号，如果是shortform标志的话，就要将操作数或到opcode中
    {
      if (!process_operands ()) /// 如push rbp 就是0x50|0x5=0x55
	return;
    }
  ...
  while (*l != END_OF_INSN)
	...
	{			/* Yes, we've read in another operand.  */
	  ...
	  if (intel_syntax)
	    ...
	    operand_ok = i386_att_operand (token_start);  /// xie: 从token_start里拿出来reloc
...
build_modrm_byte (void)
{
  ...
  if (i.mem_operands == 0
      ...
      if (i.mem_operands)
	{
	  ...
	  default_seg = &ds;
        /// xie: 检查基址，其中涉及到检查基址是否为eip或rip，以给i.op[0]设置为PCREL
	  if (i.base_reg == 0)
	    {
	      i.rm.mode = 0;
	      if (!i.disp_operands)
...
output_branch (void)
{
  ...
  *p = i.tm.base_opcode; /// xie: 写入jmp指令的操作码
  ...
  /* 1 possible extra opcode + 4 byte displacement go in var part.
    ...
    /// xie: 该函数创建新的frag，并存储fixup的sym和off信息。在tc-i386中跳转指令就会走到这里
    /// 但注意在tc-aarch64中，没有对frag_var的直接调用，也就是说没有指令会将其fixup信息写到frag中。也就是说第二类fixup在aarch64中不存在？
...
output_jump (void)
{
  ...
  jump_reloc = reloc (size, 1, 1, jump_reloc);
    ...
    /// xie: 这是call指令中的fixup,可以看到其pcrel默认也是1,也就是call f1,f1会以相对地址表示
...
output_insn (void)
{
  ...
  /* Output jumps.  */
  if (i.tm.opcode_modifier.jump == JUMP) /// xie: jmp指令走这里
    output_branch ();
  else if (i.tm.opcode_modifier.jump == JUMP_BYTE /// xie: call指令走这里
	   || i.tm.opcode_modifier.jump == JUMP_DWORD)
    output_jump ();
      ...
      if (!i.tm.opcode_modifier.vex && !i.tm.opcode_modifier.evex)
	{
	  switch (i.tm.opcode_modifier.opcodeprefix)
		...
		case 3:
		case 2:
		case 1: /// xie: push这种字节码长度为1的，就不用改动前缀
		  break;
		case 0:
	  ...
	  /* The prefix bytes.  */
          /// xie: 写汇编指令的前缀，包括rex prefix 和 prefix两部分
          /// 指令的rex prefix，对于mov rsp, rbp来说，因为是64位操作数，所以rex prefix存在，为0100 1000
          /// 当然，汇编指令还有prefix前缀部分，但该指令没有
	...
	abs_section_offset += i.tm.opcode_length;
      // xie: 字节码长度为1的，直接加入该字节
      else if (i.tm.opcode_length == 1)
	{
	  FRAG_APPEND_1_CHAR (i.tm.base_opcode);
	}
      /// xie: 字节码长度大于1的，低2位在switch外面放入，高位在switch里放入。这里frag_more应该是申请长度，返回的p是基址，先放入>>24高位
      else
	{
	  switch (i.tm.opcode_length)
	    {
	  ...
	  *p = i.tm.base_opcode & 0xff;
      ...
      /* Now the modrm byte and sib byte (if present).  */
        /// xie: 写入MODR/M和SIB两个字节，MODR/M存放在i.rm里。SIB存放在i.sib里
      if (i.tm.opcode_modifier.modrm)
	{
	  ...
	  if (i.rm.regmem == ESCAPE_TO_TWO_BYTE_ADDRESSING
	    ...
	    frag_opcode_byte ((i.sib.base << 0) /// xie: SIB的三个部分也是一样，都准备好了
			      | (i.sib.index << 3)
			      | (i.sib.scale << 6));
	}

      if (i.disp_operands)  /// xie: 写displacement部分
	output_disp (insn_start_frag, insn_start_off);

      if (i.imm_operands)   /// xie: 写immediate部分
...
output_disp (fragS *insn_start_frag, offsetT insn_start_off)
{
  ...
  unsigned int n;
    ...
    /// xie: 它的displacement是以操作数存在op里的，其实也就是用displacement指代所有形式的操作数。
    /// 所以要遍历i.types，检查有没有disp类型的操作数
	  ...
	  if (now_seg == absolute_section)
	  ...
	  else if (i.op[n].disps->X_op == O_constant)
	    {
	      ...
	      val = offset_in_range (val >> (size == 1 ? i.memshift : 0),
				     size);
	      p = frag_more (size); /// xie: 这里是预先确认obstack的chunk中含有足够的空间
	      md_number_to_chars (p, val, size);  /// xie: p就是frchain.obstack.next_free，向后写入就可以了
	    }
      /// xie: 像mov [rip+.LC0], rdx这种，displacement表达式中含有symbol，就要考虑用fixup来处理
	  else
	    {
          ...
          /// xie: 对于leaq	.LC1(%rip), %rdi   和    leaq	.LC0(%rdi), %rdi 之所以汇编器对两者的理解不同，是出于以下原因：
          /// 首先我们现在处于写displacement环节，也就是括号左边的部分。
          /// 这部分可以是一个符号，也就是i.op[0].disp里可以是个symbol，同时一下两种方式会为该符号的定位提供base
          /// 1. 括号里面，也就会base_reg如果是rip或者eip的话，会为i.flags[0]做个1的标记，然后该符号就知道自己是要以pc作为base了。
          /// 2. leaq	.LC1-.LFB0+4(%rdx), %rdi 也是可以编译通过的，这时候就表示该符号是以.LFB0作为base，以4作为add。
          ///
          /// 但奇怪的是leaq	.LC1-.LFB0+4(%rip), %rdi也可以编译通过，这时候其实.LFB0和rip都提供了base，那么选择谁呢？？
          /// fixup的type里存的是PCRel，但仍然有base_symbol，这要看它重定位的加数是怎么写的了，目前还看不出来。
	      ...
	      else if (align_branch_power)
		{
          ...
          /// xie: 在i.op[n]为disp时，且i.op[n].disps.op->X_op非O_constant时，就要写入fixup了
                 ...
                 fragS *fragP)
{
  ...
  /* Now put displacement after opcode.  */
    /// xie: 记录跳转指令的fix信息到frag里面去
    /// 因为jump指令的，它应该都是默认用PC来计算的，所以
    /// pc + fixup = target
    /// opcode_address + extension + fixup = target_address
    /// fixup = target_address - opcode_address - extension
    /// 其中opcode_address是该不确定位置开始的地方，extension一共几个不确定字节，包括可能存在的opcode以及fixup，因此opcode_addresss+extension=pc
  md_number_to_chars ((char *) where_to_put_displacement,
		      (valueT) (displacement_from_opcode_start - extension),
		      DISP_SIZE_FROM_RELAX_STATE (fragP->fr_subtype));

    /// xie: 收集第二类fixup
    if (!is_special_section_for_fixup(sec->name)) {

        /// 初始化fixupTuple
        ShuffleInfo__Metadata__FixupInfo* fixupTuple = malloc(sizeof(ShuffleInfo__Metadata__FixupInfo));
        memset(fixupTuple, 0, sizeof(ShuffleInfo__Metadata__FixupInfo));
        unsigned int size = extension - add_opcode;

        shuffle_info__metadata__fixup_info__init(fixupTuple);
        fixupTuple->has_add = 1;
        fixupTuple->has_step = 1;

        fixupTuple->offset = opcode_address + add_opcode;
        fixupTuple->add = 0;
        fixupTuple->step = 1;

        unsigned type = 0;
        /// 记录target的符号，因为该fixup放到了add_jmp_fixup里所以在符号表确定后还会有一次处理
        if (fragP->fr_symbol)
            fixupTuple->target_bbl_sym = fragP->fr_symbol;

        /// 记录type  0-1base&2-3target->(Out|PC|VALUE|INDEX) 4->isRela 5->is_new_section 6->isJump 7->isRAND 8->isRELOC [16:31]->reloc_type
        type |= NONE_BASE;            /// type.base_type    没有base符号，因此base_type为NONE
        type |= NONE_TARGET << 2;     /// type.target_type  现在只记录了target符号，还没有判断，因此targetType为NONE,后面会再改
        if (size == 1)/// type.reloc_type   默认是PC-relative的
            type |= 15 << 16; // R_X86_64_PC8
        else if(size == 2)
            type |= 13 << 16; // R_X86_64_PC16
        else if(size == 4)
            type |= 2 << 16;  // R_X86_64_PC32
        else if(size == 8)
            type |= 24 << 16; // R_X86_64_PC64

        fixupTuple->type = type;

        add_fixup(sec->name, fixupTuple);
        add_jmp_fixup(sec->name, fixupTuple, sec);
    }
...
void
md_apply_fix (fixS *fixP, valueT *valP, segT seg ATTRIBUTE_UNUSED) /// xie: p是写入的位置，因为frag->fr_literal代表了该frag存储数据的部分
{
  char *p = fixP->fx_where + fixP->fx_frag->fr_literal;
  valueT value = *valP; /// xie: value是要写入的值，给其赋值上初始值valP，其实就是fixup.add_number
  ...
  *valP = value;
  ...
  if (fixP->fx_addsy == NULL) /// xie: 能够计算出来的话(存储target的addsy为NULL)，则写入计算结果
    fixP->fx_done = 1;
#if defined (OBJ_COFF) && defined (TE_PE)
...
#endif
  else if (use_rela_relocations) /// xie: 没有计算出来的话，则写入0
    {
