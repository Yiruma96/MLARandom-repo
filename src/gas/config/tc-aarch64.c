...
s_ltorg (int ignored ATTRIBUTE_UNUSED)
{
  ...
  for (align = 2; align <= 4; align++)
	...
	frag_align (align, 0, 0);
      ...
      /// xie: 需要新建的间接引用都放在pool里面，可以看下当初是什么指令会下往池子里emit这种间接引用
      for (entry = 0; entry < pool->next_free_entry; entry++)
	{
...
s_tlsdesccall (int ignored ATTRIBUTE_UNUSED)
{
  ...
  frag_grow (4);
  /// xie. 给binutils的修复建议是将size设置为0，因为这本来就不是一个实体的指针，会在开启listing时候超过frag->size的边界。
  fix_new_aarch64 (frag_now, frag_more (0) - frag_now->fr_literal, 0, &exp, 0,
...
/// xie: 这是一个二维表，通过修饰符和操作码，来得到操作数的重定位类型
...
/* Parse an operand for an ADRP instruction:
   ...
   Return TRUE on success; otherwise return FALSE.  */
/** xie:
 * 为什么ARM里的重定位类型多呢？ 因为ARM的重定位和指令的基址，mask，位移，存储都有关系
 * 其中mask，存储信息来自指令    而基址，位移来自修饰符
 * 而X86的重定位只和基址，长度有关，因此读取修饰符就行了。而ARM的重定位需要组织一张表，综合指令和修饰符来确定重定位类型
 * */
 /// 以parse_adrp为例子，介绍gas是怎么解析汇编指令中的操作数的
static bfd_boolean
parse_adrp (char **str)
{
    /// xie: 操作数可能是.LC1  或者是  :lo12:.LC2，后者是包括修饰符在内的
    /// 解析操作数的步骤分为两步，1. 首先识别修饰符并得知重定位类型
  char *p;

    /// xie: 1. 以:作为识别修饰符的标志，有的话则综合修饰符和opcode来从reloc_table_entry中得到重定位类型
  p = *str;
  if (*p == ':')
    {
      struct reloc_table_entry *entry;
      ...
      if (entry->adrp_type == 0)
	{
	  ...
	  return FALSE;
        ...
        /// xie: 1.2. 从entry中基于opcode得到重定位类型
      inst.reloc.type = entry->adrp_type;
    }
  else
      /// xie: 1. 没有的话直接通过指令确定重定位类型
    inst.reloc.type = BFD_RELOC_AARCH64_ADR_HI21_PCREL;

  inst.reloc.pc_rel = 1;

  /// xie: 2. 然后解析该操作数到inst.operation中
  if (! my_get_expression (&inst.reloc.exp, &p, GE_NO_PREFIX, 1))
    return FALSE;
...
output_inst (struct aarch64_inst *new_inst)
{
  ...
  frag_now->tc_frag_data.recorded = 1;
    /// xie: 把存在aarch64_inst.value里的opcode写入字节序列
  put_aarch64_insn (to, inst.base.value);

    /// xie:根据是否有重定位，记录到fixup中。
    /// 需要注意的是，x64是在output_disp和output_imm的时候根据是否可计算来记录fixup的，这里是根据是否有重定位。
    /// 也就是说这个是否要生成fixup的判断是由生成何种重定位做的。如果一个偏移量可以当场计算，那么就没必要生成fixup了
  if (inst.reloc.type != BFD_RELOC_UNUSED)
    { /// xie: 和我的(frchain_now->frch_obstack.next_free) - &(frag_now->fr_literal[0])类似，但注意结果只是frag内的地址，所以还是需要frag_now的
...
parse_operands (char *str, const aarch64_opcode *opcode)
{
  ...
  if (AARCH64_CPU_HAS_FEATURE (AARCH64_FEATURE_SVE, *opcode->avariant))
    ...
    /// xie: 首先传入的str是包含有操作数！  另外aarch64_inst.operands中记录的6个操作数的type。
    /// 其实就是根据operand的type去解析str的每一部分，解析结果填回到operand中，另外会产生fixup的操作数则会记录到inst.reloc中
...
/// xie:基于指令的opcode选择模板template   剩下的p就是操作数了，所以接下来要去解析操作数
  ...
  reset_aarch64_instruction (&inst); /// xie: 重置全局变量inst
  inst.cond = saved_cond;

  /* Iterate through all opcode entries with the same mnemonic name.  */
    /// xie:template可能是一个列表，所以这里会遍历所有的template以找到合适的输出
  do
    {
	...
	dump_opcode_operands (opcode);
      ...
      inst_base = &inst.base;
      inst_base->opcode = opcode; /// 所有templates的opcode是一样的，所以inst_base可以直接接受template.opcode的值
    ...
    /// xie: 解析操作数到inst   另外这里有个筛选模板的重要操作do_encode，为false的都不会产生最终的output
	  ...
	  && do_encode (inst_base->opcode, &inst.base, &inst_base->value))
	{
	  ...
	  if (inst.reloc.type == BFD_RELOC_UNUSED
	  ...
	  else
	    {
	      ...
	      memcpy (copy, &inst.base, sizeof (struct aarch64_inst)); /// xie: 保存一份inst.aarch64_inst，为了后面的fixup
	      /// 输出inst.base
	      output_inst (copy);
	    }
